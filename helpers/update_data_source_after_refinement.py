        ...
        with open(os.path.join(refinement_directory, 'residue_scores.csv'), 'rb') as csv_import:
            ...
            for i, line in enumerate(csv_dict):
                ...
                if len(residue.split('-')) == 2:
                    ...
                    residue_xyz = pdbtools(os.path.join(inital_model_directory, xtal, 'refine.pdb')
                                           ).get_center_of_gravity_of_residue_ish(residue_chain, residue_number)
                    event = db.execute_statement(
                        "select PANDDA_site_x,PANDDA_site_y,PANDDA_site_z,PANDDA_site_index from panddaTable where CrystalName='{0!s}'".format(xtal))
                    for coord in event:
                        db_pandda_dict = {}
...
def update_ligand_information_in_panddaTable(inital_model_directory, xtal):
    if os.path.isfile(os.path.join(inital_model_directory, xtal, 'refine.pdb')):
        ...
        ligands_in_file = pdbtools(os.path.join(
            inital_model_directory, xtal, 'refine.pdb')).get_residues_with_resname('LIG')
...
def update_buster_report_index_html(refinement_directory, db_dict):
    if os.path.isfile(refinement_directory+'-report/index.html'):
        db_dict['RefinementBusterReportHTML'] = refinement_directory + \
            '-report/index.html'
    return db_dict
    ...
    if os.path.isfile(os.path.join(inital_model_directory, xtal, 'refine.pdb')) and \
       ...
       os.path.isfile(os.path.join(inital_model_directory, xtal, 'fofc.map')):
        ...
        for ligand in ligandDict:
            ...
            os.system('/bin/mv 2fofc_mapmask.map %s_%s_2fofc_cut.ccp4' %
                      (xtal, ligand))
            maptools().cut_map_around_ligand('fofc.map', ligand+'.pdb', '7')
            os.system('/bin/mv fofc_mapmask.map %s_%s_fofc_cut.ccp4' %
                      (xtal, ligand))
