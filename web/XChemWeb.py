                ...
                if self.db_dict['ProteinName'] == 'None':
                    ...
                    try:
                        ...
                        self.Logfile.warning(
                            'xtal name = %s => setting protein name to %s' % (xtal, self.protein_name))
                    except IndexError:
                        self.Logfile.warning(
                    ...
                    self.Logfile.insert(
                        'protein name is: ' + self.protein_name)
            self.copy_pdb(xtal)
            self.copy_mtz(xtal)
...
#                self.cut_and_copy_map(xtal, ligand + '.pdb', 'fofc.map', DELFWT,'DELFWT','PHDELWT')
                ...
                if ligConfidence.startswith('0'):
                    self.Logfile.warning('%s: ligand confidence of %s-%s-%s is %s; ignoring...' % (
                        xtal, ligChain, ligNumber, ligName, ligConfidence))
                    self.Logfile.warning(
                        '%s: this seems unlikely because this structure is apparently ready for deposition' % xtal)
                    self.Logfile.warning(
                        '%s: will set it to "not assigned" for now, but please update in soakDB' % xtal)
                    ligConfidence = 'not assigned'
                modelStatus = self.db_dict['RefinementOutcome']
                if firstFile:
                    html += XChemMain.html_ngl(pdb, eventMap.replace(
                        self.projectDir, ''), FWTmap, DELFWTmap, ligand)
                ...
                html += XChemMain.html_table_row(xtal, pdbID, ligand, compoundImage, residuePlot, pdb, event,
                                                 thumbNail, resoHigh, spg, unitCell, FWTmap, DELFWTmap, ligConfidence, modelStatus)
                self.make_thumbnail(xtal, x, y, z, ligand, eventMap)
                self.prepare_for_download(
                    xtal, pdb, event, compoundCIF, ligand)
...
#        html = XChemMain.html_download_all_section(html,self.protein_name)
    ...
    def prepare_e_density_maps(self, xtal, ligand):
        ...
        if os.path.isfile('refine_mapmask.ccp4'):
            self.Logfile.insert(xtal + ': 2fofc map around ' +
                                ligand + ' successfully created')
        else:
            self.Logfile.error(xtal + ': 2fofc map around ' +
        ...
        self.Logfile.insert(xtal + ': current 2fofc map -> ' +
                            xtal + '_' + ligand + '_2fofc.ccp4')
    ...
    def prepare_for_download(self, xtal, pdb, event, compoundCIF, ligID):
        ...
        if zip_in != '':
            self.Logfile.insert(
                '%s: preparing zip file -> zip %s_%s.zip %s' % (xtal, xtal, ligID, zip_in))
    ...
    def copy_jscss(self):
        ...
        os.system('/bin/cp -r %s/web/jscss/css .' %
                  os.getenv('XChemExplorer_DIR'))
        os.system('/bin/cp -r %s/web/jscss/js .' %
                  os.getenv('XChemExplorer_DIR'))
    ...
    def make_thumbnail(self, xtal, x, y, z, ligID, eventMap):
        ...
        if not os.path.isfile('%s_%s_thumb.png' % (xtal, ligID)):
            self.Logfile.insert(
                '%s: preparing thumbnail image of %s' % (xtal, ligID))
...
#            os.mkdir('js')
    ...
    def copy_pdb(self, xtal):
        ...
        if os.path.isfile(os.path.join(self.projectDir, xtal, 'refine.split.bound-state.pdb')):
            self.pdb = pdbtools(os.path.join(
                self.projectDir, xtal, 'refine.split.bound-state.pdb'))
            self.Logfile.insert(
            ...
            self.Logfile.error(
                '%s: cannot find refine.split.bound-state.pdb' % xtal)
    ...
    def copy_electron_density(self, xtal, ligand, eventMap):
        ...
        if os.path.isfile(os.path.join(self.projectDir, xtal, emap)):
            self.Logfile.insert(
                '%s: copying %s to html directory' % (xtal, emap))
            os.system('/bin/cp %s/%s %s' %
                      (os.path.join(self.projectDir, xtal), emap, emap))
        else:
        ...
        self.Logfile.insert('%s: looking for %s' % (
            xtal, os.path.join(self.projectDir, xtal, eventMap)))
        if os.path.isfile(os.path.join(self.projectDir, xtal, eventMap)):
            self.Logfile.insert(
                '%s: copying %s to html directory' % (xtal, eventMap))
            os.system('/bin/cp %s/%s %s' %
    ...
    def copy_ligand_files(self, xtal):
        ...
        if os.path.isfile(os.path.join(self.projectDir, xtal, self.db_dict['CompoundCode']+'.cif')):
            ...
            os.system('/bin/cp %s %s_%s' % (os.path.join(self.projectDir, xtal,
                                                         self.db_dict['CompoundCode']+'.cif'), xtal, self.db_dict['CompoundCode']+'.cif'))
        else:
            self.Logfile.error('%s: cannot find compound cif file -> %s' %
        ...
        if os.path.isfile(os.path.join(self.projectDir, xtal, self.db_dict['CompoundCode']+'.png')):
            ...
            os.system('/bin/cp %s %s_%s' % (os.path.join(self.projectDir, xtal,
                                                         self.db_dict['CompoundCode']+'.png'), xtal, self.db_dict['CompoundCode']+'.png'))
        else:
            self.Logfile.error('%s: cannot find compound png file -> %s' %
    ...
    def copy_spider_plot(self, xtal, ligID):
        ...
        if os.path.isfile(refPDB):
            refPDBreal = os.path.realpath(
                refPDB)[:os.path.realpath(refPDB).rfind('/')]
            plot = os.path.join(refPDBreal, 'residue_plots',
                                ligID.replace('LIG-', '')+'.png')
            self.Logfile.insert(xtal + ': looking for ' + plot)
            if os.path.isfile(plot):
                self.Logfile.insert('%s: found %s' % (xtal, plot))
                self.Logfile.insert('%s: copying spider plot for %s' % (
                    xtal, ligID.replace('LIG-', '') + '.png'))
                os.system('/bin/cp %s %s_%s.png' % (plot, xtal, ligID))
            else:
                self.Logfile.error('%s: cannot find spider plot for %s' % (
                    xtal, ligID.replace('LIG-', '')+'.png'))
                self.Logfile.warning('%s: using %s instead' % (
                    xtal, 'NO_SPIDER_PLOT_AVAILABLE.png'))
                os.system('/bin/cp %s %s_%s.png' % (os.path.join(os.getenv(
                    'XChemExplorer_DIR'), 'image', 'NO_SPIDER_PLOT_AVAILABLE.png'), xtal, ligID))
                #
        else:
            self.Logfile.error(
                '%s: cannot find refine.pdb, i.e. cannot start looking for spider plots...' % xtal)
