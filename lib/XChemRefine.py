...
class RefineParams(object):
    ...
    def RefmacRefinementParams(self, RefmacParams):
        ...
        self.TLS = gtk.CheckButton(
            'TLS (find TLS groups with phenix.find_tls_groups)')
        ...
        if os.path.isfile(self.ProjectPath+'/'+self.xtalID+'/Refine_'+str(Serial)+'/refmac.csh'):
            for line in open(self.ProjectPath+'/'+self.xtalID+'/Refine_'+str(Serial)+'/refmac.csh'):
                ...
                if line.startswith('weight'):
                    RefmacParams['MATRIX_WEIGHT'] = line.split()[
                        len(line.split())-1]
        ...
        if found:
            for cycle in sorted(RefinementCycle):
                ...
                try:
                    found_Rcryst = False
                    found_Rfree = False
                    newestPDB = max(glob.iglob(self.ProjectPath+'/'+self.xtalID+'/Refine_'+str(
                        cycle)+'/refine_'+str(cycle)+'.pdb'), key=os.path.getctime)
...
class Refine(object):
    ...
    def make_covalent_link(self, covLinkAtomSpec, Logfile):
        ...
        if residue_2 in residuesToModify:
            bond_text = 'LINK: RES-NAME-1 %s FILE-1 %s_acedrg.cif ATOM-NAME-1  %s  RES-NAME-2 %s ATOM-NAME-2  %s' % (
        ...
        os.system('acedrg -c %s.cif -o %s_acedrg' %
                  (self.compoundID, self.compoundID))

        os.system('acedrg -L covalent_bond.txt -o %s' % self.compoundID)

        if os.path.isfile('%s_link.cif' % self.compoundID):
            Logfile.insert(
                'succefully created link file %s_link.cif' % self.compoundID)
            os.system('/bin/rm %s.cif' % self.compoundID)
            os.system('ln -s %s_link.cif %s.cif' %
                      (self.compoundID, self.compoundID))
        else:
            Logfile.error('could not create linkfile; please check logs in %s/%s' %
...
#            RefmacParams['HKLIN']='HKLIN '+os.path.join(self.ProjectPath,self.xtalID,self.xtalID+'-pandda-input.mtz \\\n')
            ...
            self.Logfile.error(
                '%s: cannot find HKLIN for refinement' % self.xtalID)
            self.error = True
        hklout = os.path.join(self.ProjectPath, self.xtalID,
                              'Refine_'+Serial, 'refine_'+Serial+'.mtz')
        return hklin, hklout
    ...
    def get_xyzin_xyzout(self, Serial):
        ...
        if os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, 'Refine_'+Serial, 'in.pdb')):
            ...
            xyzout = os.path.join(
                self.ProjectPath, self.xtalID, 'Refine_'+Serial, 'refine_'+Serial+'.pdb')
        elif os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial), 'in.pdb')):
            xyzin = os.path.join(self.ProjectPath, self.xtalID,
                                 'cootOut', 'Refine_'+str(Serial), 'in.pdb')
        else:
            self.Logfile.error(
                '%s: cannot find XYZIN for refinement' % self.xtalID)
            self.error = True
        return xyzin, xyzout
    ...
    def get_libin_libout(self, Serial):
        ...
        if os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, self.compoundID+'.cif')):
            ...
            libout = os.path.join(
                self.ProjectPath, self.xtalID, 'Refine_'+Serial, 'refine_'+Serial+'.cif')
        else:
            self.Logfile.error(
                '%s: cannot find CIF file for refinement' % self.xtalID)
            self.error = True
        return libin, libout
    ...
    def get_source_line(self, cmd):
        if 'bash' in os.getenv('SHELL'):
            cmd += 'export XChemExplorer_DIR="' + \
                os.getenv('XChemExplorer_DIR')+'"\n'
        elif 'csh' in os.getenv('SHELL'):
            cmd += 'setenv XChemExplorer_DIR ' + \
                os.getenv('XChemExplorer_DIR')+'\n'
        return cmd
    ...
    def run_script(self, program, external_software):
        ...
        if external_software['qsub'] and os.path.isdir('/dls'):
            self.Logfile.insert(
                'starting refinement with command: qsub -P labxchem -q medium.q %s.sh' % program)
            ...
            self.Logfile.insert(
                'starting refinement with command: %s' % remote_command)
            os.system('%s -P labxchem refmac.csh' % remote_command)
        else:
            self.Logfile.insert(
                'starting refinement with command: ./%s.sh &' % program)
    ...
    def RunBuster(self, Serial, RefmacParams, external_software, xce_logfile, covLinkAtomSpec):
        ...
        if os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, 'REFINEMENT_IN_PROGRESS')):
            self.Logfile.insert(
                'cannot start new refinement for %s: *** REFINEMENT IN PROGRESS ***' % self.xtalID)
            return None
        ...
        cmd = self.add_buster_command(cmd, xyzin, hklin, libin, Serial, anisotropic_Bfactor,
                                      update_water, refine_ligand_occupancy, ignore_sanity_check)
        ...
        if self.error:
            ...
            self.Logfile.insert('%s: writing buster.sh file in %s' % (
...
#            RefmacParams['HKLIN']='HKLIN '+os.path.join(self.ProjectPath,self.xtalID,self.xtalID+'-pandda-input.mtz \\\n')
        ...
        RefmacParams['HKLOUT'] = 'HKLOUT '+os.path.join(
            self.ProjectPath, self.xtalID, 'Refine_'+Serial, 'refine_'+Serial+'.mtz \\\n')

        #######################################################
        # XYZIN & XYZOUT
        RefmacParams['XYZIN'] = 'XYZIN ' + \
            os.path.join(self.ProjectPath, self.xtalID,
                         'Refine_'+Serial, 'in.pdb \\\n')
        ...
        found_cif = False
        if os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, self.compoundID+'.cif')):
            RefmacParams['LIBIN'] = 'LIBIN '+self.ProjectPath + \
                '/'+self.xtalID+'/'+self.compoundID+'.cif \\\n'
            RefmacParams['LIBOUT'] = 'LIBOUT '+self.ProjectPath+'/' + \
                self.xtalID+'/Refine_'+Serial+'/refine_'+Serial+'.cif \\\n'
        ...
        if RefmacParams['TLS'].startswith('refi'):
            if external_software['phenix.find_tls_groups']:
                findTLS = os.path.join(os.getenv(
                    'XChemExplorer_DIR'), 'helpers', 'phenix_find_TLS_groups.py')+' in.pdb\n'
                RefmacParams['TLSIN'] = 'TLSIN '+self.ProjectPath + \
                    '/'+self.xtalID+'/Refine_'+Serial+'/refmac.tls \\\n'
                RefmacParams['TLSOUT'] = 'TLSOUT '+self.ProjectPath + \
                    '/'+self.xtalID+'/Refine_'+Serial+'/refine.tls \\\n'
        ...
        if 'bash' in os.getenv('SHELL'):
            source = (
                ...
                os.getenv('XChemExplorer_DIR')+'"\n'
                '\n'
                'source '+os.path.join(os.getenv('XChemExplorer_DIR'),
                                       'setup-scripts', 'xce.setup-sh')+'\n'
        ...
        if os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, self.xtalID+'-ensemble-model.pdb')):
            if os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, self.xtalID+'-pandda-input.mtz')):
                ...
                pdb_one = os.path.join(
                    self.ProjectPath, self.xtalID, 'Refine_'+str(Serial), 'refine_'+str(Serial)+'.pdb')
                mtz_one = os.path.join(
                    self.ProjectPath, self.xtalID, 'Refine_'+str(Serial), 'refine_'+str(Serial)+'.mtz')
                spider_plot = (
        ...
        if os.path.isfile(xce_logfile):
            Logfile.insert('writing refinement shell script to'+os.path.join(
                self.ProjectPath, self.xtalID, 'Refine_'+Serial, 'refmac.csh'))
        cmd = open(os.path.join(self.ProjectPath, self.xtalID,
                                'Refine_'+Serial, 'refmac.csh'), 'w')
...
#        os.system('ssh artemis "cd %s/%s/Refine_%s; qsub refmac.csh"' %(self.ProjectPath,self.xtalID,Serial))
        ...
        if external_software['qsub_remote'] != '':
            ...
            Logfile.insert(
                'starting refinement on cluster with command "qsub -P labxchem -q medium.q refmac.csh"')
...
class panddaRefine(object):
    ...
    def RunQuickRefine(self, Serial, RefmacParams, external_software, xce_logfile, refinementProtocol, covLinkAtomSpec):
        ...
        Logfile.insert(
            'trying to merge modified bound state with ground state')
        if os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial), self.xtalID+'-ensemble-model.pdb')):
            Logfile.insert(
                'seems to be an initial refinement after pandda.export, no need to merge the conformations')
            os.chdir(os.path.join(self.ProjectPath, self.xtalID,
                                  'cootOut', 'Refine_'+str(Serial)))
            Logfile.insert('running giant.make_restraints %s:' %
...
#            os.system('giant.make_restraints %s' %self.xtalID+'-ensemble-model.pdb')
            ...
            Logfile.insert('found model of ground state: '+os.path.join(
            ...
            if os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial), 'refine.modified.pdb')):
                ...
                Logfile.insert('running giant.merge_conformations major=%s minor=%s' % (
                ...
                if os.getcwd().startswith('/dls'):
                    cmd = (
                        ...
                        'giant.merge_conformations major=%s minor=%s reset_all_occupancies=False options.major_occupancy=1.0 options.minor_occupancy=1.0' % (
                            ground_state, bound_state)
                    )
                else:
                    cmd = (
                ...
                Logfile.error('cannot find modified version of bound state in %s' % os.path.join(
                    self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial)))
                return None
        else:
            if os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial), 'refine.modified.pdb')):
                os.chdir(os.path.join(self.ProjectPath, self.xtalID,
                                      'cootOut', 'Refine_'+str(Serial)))
                Logfile.warning(
                    '%s: ground-state model does not exist, but refine.modified.pdb does exist' % self.xtalID)
                Logfile.warning(
                    'This may be a case where there are no differences between bound and ground state')
                Logfile.warning(
                    'creating symbolic link: ln -s refine.modified.pdb %s-ensemble-model.pdb' % self.xtalID)
                os.system(
        ...
        if os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial), self.xtalID+'-ensemble-model.pdb')):
            RefmacParams['XYZIN'] = os.path.join(
                self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial), self.xtalID+'-ensemble-model.pdb')
        elif os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial), 'multi-state-model.pdb')):
            RefmacParams['XYZIN'] = os.path.join(
                self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial), 'multi-state-model.pdb')
        else:
            Logfile.error('cannot find multi-state-model.pdb in %s; aborting...' %
                          os.path.join(self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial)))
            return None
        ...
        if os.path.isfile(os.path.join(self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial), 'multi-state-restraints.refmac.params')):
            ...
            Logfile.warning('cannot find multi-state-restraints.refmac.params in %s!' %
                            os.path.join(self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial)))
            try:
                os.chdir(os.path.join(self.ProjectPath, self.xtalID,
                                      'cootOut', 'Refine_'+str(Serial)))
                Logfile.insert(
                    'checking if %s-ensemble-model.pdb contains residue of type LIG, DRG, FRG, UNK or UNL' % self.xtalID)
                knowLigandIDs = ['LIG', 'DRG', 'FRG', 'UNK', 'UNL']
                ligandsInFile = pdbtools(
                    self.xtalID+'-ensemble-model.pdb').find_ligands()
                found_lig = False
                ...
                if found_lig:
                    ...
                    Logfile.error(
                        '%s-ensemble-model.pdb does not contain any modelled ligand; aborting refinement' % self.xtalID)
                    return None
            except OSError:
                Logfile.error('directory does not exisit: %s' % os.path.join(
                    self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial)))
        ...
        if refinementProtocol == 'pandda_refmac':
            ...
            refinementParams = os.path.join(
                self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial), 'multi-state-restraints.refmac.params')
            mapCalculation = (
                'fft hklin refine.mtz mapout 2fofc.map << EOF\n'
                'labin F1=FWT PHI=PHWT\n'
                'EOF\n'
            ...
            refinementParams = os.path.join(
                self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial), 'multi-state-restraints.phenix.params')
            mapCalculation = (
                'fft hklin refine.mtz mapout 2fofc.map << EOF\n'
        ...
        refmacCmds = (
            ...
            'ln -s '+self.ProjectPath+'/'+self.xtalID+'/Refine_' +
            str(panddaSerial)+'/refine_'+str(Serial)+'_001.pdb '
            + self.ProjectPath+'/'+self.xtalID+'/Refine_' +
            str(panddaSerial)+'/refine_'+str(Serial)+'.pdb' + '\n'

            'ln -s '+self.ProjectPath+'/'+self.xtalID+'/Refine_' +
            str(panddaSerial)+'/refine_'+str(Serial)+'_001.mtz '
            + self.ProjectPath+'/'+self.xtalID+'/Refine_' +
            str(panddaSerial)+'/refine_'+str(Serial)+'.mtz' + '\n'
        ...
        Logfile.insert('writing refinement shell script to'+os.path.join(
            self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial), 'refmac.csh'))
        cmd = open(os.path.join(self.ProjectPath, self.xtalID,
                                'cootOut', 'Refine_'+str(Serial), 'refmac.csh'), 'w')
        cmd.write(refmacCmds)
        cmd.close()
        ...
        os.chdir(os.path.join(self.ProjectPath, self.xtalID,
                              'cootOut', 'Refine_'+str(Serial)))
#        os.system('ssh artemis "cd %s/%s/Refine_%s; qsub refmac.csh"' %(self.ProjectPath,self.xtalID,Serial))
        Logfile.insert('changing directory to %s' % (os.path.join(
            self.ProjectPath, self.xtalID, 'cootOut', 'Refine_'+str(Serial))))
