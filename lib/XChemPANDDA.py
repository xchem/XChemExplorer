...
class convert_all_event_maps_in_database(QtCore.QThread):
    ...
    def run(self):
        ...
        for item in query:
            ...
            if os.path.isfile(
                os.path.join(self.initial_model_directory, xtalID, "refine.pdb")
            ):
                ...
                if os.path.isfile(
                    "ligand_{0!s}_{1!s}_{2!s}_{3!s}.pdb".format(
                        str(resname), str(chainID), str(resseq), str(altLoc)
                    )
                ):
                ...
                self.Logfile.insert(
                    "directory: "
                    + os.path.join(self.initial_model_directory, xtalID)
                    + " -> cannot find refine.pdb; trying next"
                )
                continue

            if os.path.isfile(
                os.path.join(self.initial_model_directory, xtalID, "refine.mtz")
            ):
                resolution = XChemUtils.mtztools(
                    os.path.join(self.initial_model_directory, xtalID, "refine.mtz")
                ).get_high_resolution_from_mtz()
            else:
                self.Logfile.insert(
                    "directory: "
                    + os.path.join(self.initial_model_directory, xtalID)
                    + " -> cannot find refine.mtz; trying next"
                )
                continue
...
class convert_event_map_to_SF:
    ...
    def update_database(self):
        sqlite = (
            ...
            " PANDDA_site_event_map_mtz = '%s' "
            % os.path.join(self.project_directory, self.xtalID, self.event + ".mtz")
