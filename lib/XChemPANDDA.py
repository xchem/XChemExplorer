...
#        self.already_exported_models=[]
    ...
    def run(self):
        ...
        if os.path.isfile(os.path.join(self.PanDDA_directory, 'analyses', 'pandda_inspect_events.csv')):
            inspect_csv = pandas.read_csv(os.path.join(
        ...
        for xtal in sorted(modelsDict):
            ...
            os.chdir(os.path.join(self.PanDDA_directory,
            ...
            ligandDict = XChemUtils.pdbtools_gemmi(
                pandda_model).center_of_mass_ligand_dict('LIG')
            if ligandDict == {}:
                self.Logfile.error(
    ...
    def update_database(self, xtal, modelsDict):
        ...
        self.Logfile.insert('updating database for '+xtal +
                            ' setting time model was created to '+db_dict['DatePanDDAModelCreated'])
        self.db.update_data_source(xtal, db_dict)

    def make_and_cut_map(self, xtal, emapLigandDict):
        self.Logfile.insert('changing directory to ' +
                            os.path.join(self.project_directory, xtal))
        os.chdir(os.path.join(self.project_directory, xtal))
        XChemUtils.pdbtools_gemmi(
            xtal + '-pandda-model.pdb').save_ligands_to_pdb('LIG')
        for ligID in emapLigandDict:
            ...
            if os.path.isfile(emap.replace('.ccp4', '_mapmask.ccp4')):
                os.system('/bin/mv %s %s_%s_event.ccp4' %
                          (emap.replace('.ccp4', '_mapmask.ccp4'), xtal, ligID))
                os.system('ln -s %s_%s_event.ccp4 %s_%s_event_cut.ccp4' %
                          (xtal, ligID, xtal, ligID))

    def copy_pandda_model_to_project_directory(self, xtal):
        os.chdir(os.path.join(self.project_directory, xtal))
        model = os.path.join(self.PanDDA_directory, 'processed_datasets',
                             xtal, 'modelled_structures', xtal+'-pandda-model.pdb')
    ...
    def copy_event_mtz_to_project_directory(self, xtal):
        ...
        for emap in glob.glob('*-BDC_*.mtz'):
            self.Logfile.insert('copying %s to %s...' % (
                emap, os.path.join(self.project_directory, xtal)))
            os.system('/bin/cp %s %s' %
                      (emap, os.path.join(self.project_directory, xtal)))
    ...
    def move_old_event_to_backup_folder(self, xtal):
        ...
        for emap in glob.glob('*-BDC_*.ccp4'):
            os.system('/bin/mv %s event_map_backup/%s' % (emap, emap +
    ...
    def show_ligands_in_model(self, xtal, ligandDict):
        ...
        for lig in ligandDict:
            self.Logfile.insert(
                lig + ' -> coordinates ' + str(ligandDict[lig]))
    ...
    def find_modeled_structures_and_timestamps(self):
        ...
        for model in sorted(glob.glob(os.path.join(self.PanDDA_directory, 'processed_datasets', '*', 'modelled_structures', '*-pandda-model.pdb'))):
            sample = model[model.rfind(
                '/')+1:].replace('-pandda-model.pdb', '')
            timestamp = datetime.fromtimestamp(
                os.path.getmtime(model)).strftime('%Y-%m-%d %H:%M:%S')
            self.Logfile.insert(
    ...
    def find_new_models(self, modelsDict):
        ...
        for xtal in modelsDict:
            timestamp_file = modelsDict[xtal]
            db_query = self.db.execute_statement("select DatePanDDAModelCreated from mainTable where CrystalName is '" +
                                                 xtal+"' and (RefinementOutcome like '3%' or RefinementOutcome like '4%')")
            try:
                timestamp_db = str(db_query[0][0])
            except IndexError:
                self.Logfile.warning(
                    '%s: database query gave no results for DatePanDDAModelCreated; skipping...' % xtal)
                self.Logfile.warning(
                    '%s: this might be a brand new model; will continue with export!' % xtal)
                samples_to_export[xtal] = timestamp_file
                # some time in the future...
                timestamp_db = "2100-01-01 00:00:00"
            try:
                difference = (datetime.strptime(timestamp_file, '%Y-%m-%d %H:%M:%S') -
                              datetime.strptime(timestamp_db, '%Y-%m-%d %H:%M:%S'))
                if difference.seconds != 0:
                    self.Logfile.insert(
                        'exporting '+xtal+' -> was already refined, but newer PanDDA model available')
                    samples_to_export[xtal] = timestamp_file
                else:
                    self.Logfile.insert(
            ...
            if row['dtag'] == xtal:
                for emap in glob.glob('*-BDC_*.ccp4'):
                    ...
                    if matching_ligand is not None:
                        ...
                        self.Logfile.insert(
                            'found matching ligand (%s) for %s' % (matching_ligand, emap))
                        break
                    else:
                        self.Logfile.warning(
                            'current ligand not close to event...')
        if emapLigandDict == {}:
            self.Logfile.error(
                'could not find ligands within 7A of PanDDA events')
    ...
    def calculate_distance_to_ligands(self, ligandDict, x, y, z):
        ...
        for ligand in ligandDict:
            ...
            self.Logfile.insert('coordinates ligand: ' +
                                str(c[0])+' ' + str(c[1])+' '+str(c[2]))
            self.Logfile.insert('coordinates event:  ' +
                                str(x)+' ' + str(y)+' '+str(z))
            ...
            if os.path.isfile(os.path.join(self.project_directory, xtal, xtal+'-pandda-model.pdb')):
                ...
                if not os.path.isdir(os.path.join(self.project_directory, xtal, 'cootOut')):
                    os.mkdir(os.path.join(
                        self.project_directory, xtal, 'cootOut'))
                ...
                if os.path.isdir(os.path.join(self.project_directory, xtal, 'cootOut', 'Refine_'+str(Serial))):
                    os.chdir(os.path.join(self.project_directory,
                                          xtal, 'cootOut', 'Refine_'+str(Serial)))
                else:
                    os.mkdir(os.path.join(self.project_directory,
                                          xtal, 'cootOut', 'Refine_'+str(Serial)))
                    os.chdir(os.path.join(self.project_directory,
                                          xtal, 'cootOut', 'Refine_'+str(Serial)))
                os.system('/bin/cp %s in.pdb' %
                ...
                Refine.RunBuster(str(Serial), RefmacParams,
                                 self.external_software, self.xce_logfile, None)
            else:
                self.Logfile.error(
                    '%s: cannot find %s-pandda-model.pdb; cannot start refinement...' % (xtal, xtal))
...
class refine_bound_state_with_buster(QtCore.QThread):
    ...
    def refine_exported_models(self, samples_to_export):
        ...
        for xtal in sorted(samples_to_export):
            self.Logfile.insert(
                '%s: getting compound code from database' % xtal)
            query = self.db.execute_statement(
                "select CompoundCode from mainTable where CrystalName='%s';" % xtal)
            compoundID = str(query[0][0])
            self.Logfile.insert('%s: compounds code = %s' % (xtal, compoundID))
#            compoundID=str(item[1])
            if os.path.isfile(os.path.join(self.initial_model_directory, xtal, xtal+'.free.mtz')):
                if os.path.isfile(os.path.join(self.initial_model_directory, xtal, xtal+'-pandda-model.pdb')):
                    self.Logfile.insert(
                        'running inital refinement on PANDDA model of '+xtal)
                    Serial = XChemRefine.GetSerial(
                        self.initial_model_directory, xtal)
                    #######################################################
                    if not os.path.isdir(os.path.join(self.initial_model_directory, xtal, 'cootOut')):
                        os.mkdir(os.path.join(
                            self.initial_model_directory, xtal, 'cootOut'))
                    # create folder for new refinement cycle
                    if os.path.isdir(os.path.join(self.initial_model_directory, xtal, 'cootOut', 'Refine_'+str(Serial))):
                        ...
                        os.chdir(os.path.join(self.initial_model_directory,
                                              xtal, 'cootOut', 'Refine_'+str(Serial)))
                    os.system('/bin/cp %s in.pdb' % os.path.join(
                        self.initial_model_directory, xtal, xtal+'-pandda-model.pdb'))
                    Refine = XChemRefine.Refine(
                        self.initial_model_directory, xtal, compoundID, self.datasource)
                    Refine.RunBuster(
                        str(Serial), self.external_software, self.xce_logfile, None)
                else:
                    self.Logfile.error(
                        '%s: cannot find %s-pandda-model.pdb; cannot start refinement...' % (xtal, xtal))
    ...
    def export_models(self):
        ...
        self.Logfile.insert(
            'finding out which PanDDA models need to be exported')
        ...
        dbModelsDict = {}
        if queryModels != '':
            dbEntries = self.db.execute_statement("select CrystalName,DatePanDDAModelCreated from mainTable where CrystalName in (" +
                                                  queryModels[:-1]+") and (RefinementOutcome like '3%' or RefinementOutcome like '4%' or RefinementOutcome like '5%')")
            for item in dbEntries:
                xtal = str(item[0])
                timestamp = str(item[1])
                dbModelsDict[xtal] = timestamp
                self.Logfile.insert(
                    'PanDDA model for '+xtal+' is in database and was created on '+str(timestamp))
        ...
        if self.which_models == 'all':
            self.Logfile.insert(
                'Note: you chose to export ALL available PanDDA!')
                ...
                if sample in dbModelsDict:
                    try:
                        difference = (datetime.strptime(
                            fileModelsDict[sample], '%Y-%m-%d %H:%M:%S') - datetime.strptime(dbModelsDict[sample], '%Y-%m-%d %H:%M:%S'))
                        if difference.seconds != 0:
                            self.Logfile.insert(
                                'exporting '+sample+' -> was already refined, but newer PanDDA model available')
                            samples_to_export[sample] = fileModelsDict[sample]
                    except ValueError:
                    ...
                    self.Logfile.insert(
                        'exporting '+sample+' -> first time to be exported and refined')
                    samples_to_export[sample] = fileModelsDict[sample]
        ...
        if samples_to_export != {}:
            ...
            for sample in samples_to_export:
                self.Logfile.insert(
                    'changing directory to ' + os.path.join(self.initial_model_directory, sample))
                os.chdir(os.path.join(self.initial_model_directory, sample))
                self.Logfile.insert(sample + ': copying ' + os.path.join(self.panddas_directory,
                                                                         'processed_datasets', sample, 'modelled_structures', sample+'-pandda-model.pdb'))
                os.system('/bin/cp %s .' % os.path.join(self.panddas_directory,
                                                        'processed_datasets', sample, 'modelled_structures', sample+'-pandda-model.pdb'))
                ...
                for old_event_map in glob.glob('*-BDC_*.ccp4'):
                    if not os.path.isdir('old_event_maps'):
                        ...
                        self.Logfile.warning(
                            sample + ': moving ' + old_event_map + ' to old_event_maps folder')
                ...
                self.Logfile.insert('updating database for '+sample+' setting time model was created to ' +
                                    db_dict['DatePanDDAModelCreated']+' and RefinementOutcome to '+db_dict['RefinementOutcome'])
            ...
            if os.path.isfile(os.path.join(self.initial_model_directory, xtal, xtal+'.free.mtz')):
                if os.path.isfile(os.path.join(self.initial_model_directory, xtal, xtal+'-ensemble-model.pdb')):
                    ...
                    if os.path.isdir(os.path.join(self.initial_model_directory, xtal, 'cootOut', 'Refine_'+str(Serial))):
                        ...
                        try:
                            os.system(
                                '/bin/rm *-ensemble-model.pdb *restraints*')
                        except:
                            self.Logfile.error(
                    ...
                    Refine = XChemRefine.panddaRefine(
                        self.initial_model_directory, xtal, compoundID, self.datasource)
                    os.symlink(os.path.join(self.initial_model_directory, xtal,
                                            xtal+'-ensemble-model.pdb'), xtal+'-ensemble-model.pdb')
                    Refine.RunQuickRefine(
                        Serial, self.RefmacParams, self.external_software, self.xce_logfile, 'pandda_refmac', None)
...
#                                   ' then tell XCHEMBB ')
                    ...
                    self.Logfile.error(
                        '%s: cannot find %s-ensemble-model.pdb; cannot start refinement...' % (xtal, xtal))
                    self.Logfile.error(
    ...
    def import_samples_into_datasouce(self, samples_to_export):
        ...
        for xtal in glob.glob('*'):
            self.db.execute_statement("update mainTable set DimplePANDDAwasRun = 'True',DimplePANDDAreject = 'False',DimplePANDDApath='{0!s}' where CrystalName is '{1!s}'".format(
        ...
        try:
            ...
            for xtal in glob.glob('*'):
                self.db.execute_statement("update mainTable set DimplePANDDAwasRun = 'True',DimplePANDDAreject = 'True',DimplePANDDApath='{0!s}',DimplePANDDAhit = 'False' where CrystalName is '{1!s}'".format(
                    self.panddas_directory, xtal))
        except OSError:
            pass
        ...
        with open(os.path.join(self.panddas_directory, 'analyses', 'pandda_inspect_sites.csv'), 'rb') as csv_import:
            ...
            for i, line in enumerate(csv_dict):
                ...
                self.Logfile.insert('add to site_list_:' +
                                    str([site_index, name, comment]))
        ...
        self.Logfile.insert('reading '+os.path.join(self.panddas_directory,
                                                    'analyses', 'pandda_inspect_events.csv'))
        with open(os.path.join(self.panddas_directory, 'analyses', 'pandda_inspect_events.csv'), 'rb') as csv_import:
            ...
            for i, line in enumerate(csv_dict):
                ...
                if sampleID not in samples_to_export:
                    self.Logfile.warning(
                        '%s: not to be exported; will not add to panddaTable...' % sampleID)
                    continue
                ...
                self.Logfile.insert(
                    'reading {0!s} -> site {1!s} -> event {2!s}'.format(sampleID, site_index, event_index))
                ...
                for file in glob.glob(os.path.join(self.initial_model_directory, sampleID, '*ccp4')):
                    ...
                    if filename.startswith(sampleID+'-event_'+event_index) and filename.endswith('map.native.ccp4'):
                        event_map = file
                        self.Logfile.insert('found respective event maps in {0!s}: {1!s}'.format(
                ...
                self.db.execute_statement(
                    "update panddaTable set RefinementOutcome = '2 - PANDDA model' where CrystalName is '{0!s}' and RefinementOutcome is null".format(sampleID))
                self.db.execute_statement(
                    "update mainTable set RefinementOutcome = '2 - PANDDA model' where CrystalName is '{0!s}' and (RefinementOutcome is null or RefinementOutcome is '1 - Analysis Pending')".format(sampleID))
                self.db.execute_statement(
                    "update mainTable set DimplePANDDAhit = 'True' where CrystalName is '{0!s}'".format(sampleID))
                progress += progress_step
                self.emit(QtCore.SIGNAL('update_progress_bar'), progress)
...
class run_pandda_analyse(QtCore.QThread):
    ...
    def __init__(self, pandda_params, xce_logfile, datasource):
        ...
        self.filter_pdb = os.path.join(
            self.reference_dir, pandda_params['filter_pdb'])
        ...
        self.make_ligand_links = '$CCP4/bin/ccp4-python %s %s %s\n' % (os.path.join(os.getenv('XChemExplorer_DIR'),
                                                                                    'helpers',
                                                                                    'make_ligand_links_after_pandda.py'), projectDir, self.panddas_directory)
        ...
        if self.appendix != '':
            ...
            self.select_ground_state_model += '$CCP4/bin/ccp4-python %s %s\n' % (os.path.join(os.getenv(
                'XChemExplorer_DIR'), 'helpers', 'select_ground_state_dataset.py'), self.panddas_directory)
    ...
    def run(self):
        ...
        if os.path.isfile(os.path.join(self.panddas_directory, 'pandda.running')):
            self.Logfile.insert(
                'it looks as if a pandda.analyse job is currently running in: '+self.panddas_directory)
            ...
            source_file = ''
            source_file += 'export XChemExplorer_DIR="' + \
            ...
            if os.path.isfile(self.filter_pdb + '.pdb'):
                ...
                if self.use_remote:
                    ...
                    output = subprocess.Popen(
                        stat_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            for i in range(0, self.pandda_analyse_data_table.rowCount()):
                ignore_all_checkbox = self.pandda_analyse_data_table.cellWidget(
                    i, 7)
                ignore_characterisation_checkbox = self.pandda_analyse_data_table.cellWidget(
                    i, 8)
                ignore_zmap_checkbox = self.pandda_analyse_data_table.cellWidget(
                    i, 9)

                if ignore_all_checkbox.isChecked():
                    ignore.append(
                        str(self.pandda_analyse_data_table.item(i, 0).text()))
...
class run_pandda_two_analyse(QtCore.QThread):
    ...
    def run(self):
        ...
        if not self.data_directory.startswith('/dls'):
            self.Logfile.error(
                'Sorry, you need to be at DLS for pandda2 to work!')
            return None
...
class giant_cluster_datasets(QtCore.QThread):
    ...
    def run(self):
        ...
        if os.path.isdir('cluster_analysis'):
            self.Logfile.insert('removing old cluster_analysis directory in {0!s}'.format(
                self.panddas_directory))
            self.emit(QtCore.SIGNAL('update_status_bar(QString)'),
                      'removing old cluster_analysis directory in {0!s}'.format(self.panddas_directory))
            os.system('/bin/rm -fr cluster_analysis 2> /dev/null')
        self.Logfile.insert(
            'creating cluster_analysis directory in {0!s}'.format(self.panddas_directory))
        self.emit(QtCore.SIGNAL('update_status_bar(QString)'),
                  'creating cluster_analysis directory in {0!s}'.format(self.panddas_directory))
        ...
        self.Logfile.insert('cleaning up broken links of {0!s} and {1!s} in {2!s}'.format(
            self.pdb_style, self.mtz_style, self.initial_model_directory))
        self.emit(QtCore.SIGNAL('update_status_bar(QString)'), 'cleaning up broken links of {0!s} and {1!s} in {2!s}'.format(
            self.pdb_style, self.mtz_style, self.initial_model_directory))
        os.chdir(self.initial_model_directory)
        for xtal in glob.glob('*'):
            if not os.path.isfile(os.path.join(xtal, self.pdb_style)):
                self.Logfile.insert('missing {0!s} and {1!s} for {2!s}'.format(
        ...
        self.Logfile.insert("running giant.cluster_mtzs_and_pdbs {0!s}/*/{1!s} pdb_regex='{2!s}/(.*)/{3!s}' out_dir='{4!s}/cluster_analysis'".format(
            self.initial_model_directory, self.pdb_style, self.initial_model_directory, self.pdb_style, self.panddas_directory))
        self.emit(QtCore.SIGNAL('update_status_bar(QString)'),
...
class check_if_pandda_can_run:
    ...
    def compare_number_of_atoms_in_reference_vs_all_datasets(self, refData, dataset_list):
        ...
        for n_datasets, dataset in enumerate(dataset_list):
            if os.path.isfile(os.path.join(self.data_directory.replace('*', ''), dataset, self.pdb_style)):
                n_atom = len(pdbtools.get_pdb_as_list(os.path.join(
                    self.data_directory.replace('*', ''), dataset, self.pdb_style)))
                if n_atom_ref == n_atom:
                    self.Logfile.insert('{0!s}: atoms in PDB file ({1!s}): {2!s}; atoms in Reference file: {3!s} ===> OK'.format(
                        dataset, self.pdb_style, str(n_atom), str(n_atom_ref)))
                if n_atom_ref != n_atom:
                    self.Logfile.insert('{0!s}: atoms in PDB file ({1!s}): {2!s}; atoms in Reference file: {3!s} ===> ERROR'.format(
                        dataset, self.pdb_style, str(n_atom), str(n_atom_ref)))
            ...
            if symmDataset == symmRef:
                try:
                    difference = math.fabs(
                        1-(float(ucVolRef)/float(ucVolDataset)))*100
                    if difference < allowed_unitcell_difference_percent:
                        sampleID = dataset.replace(
                            '/'+self.pdb_style, '')[dataset.replace('/'+self.pdb_style, '').rfind('/')+1:]
    ...
    def remove_dimple_files(self, dataset_list):
        for n_datasets, dataset in enumerate(dataset_list):
            ...
            if os.path.isfile(os.path.join(self.data_directory.replace('*', ''), dataset, self.pdb_style)):
                os.system(
                    '/bin/rm '+os.path.join(self.data_directory.replace('*', ''), dataset, self.pdb_style))
                self.Logfile.insert(
            ...
            if os.path.isfile(os.path.join(self.data_directory.replace('*', ''), dataset, self.mtz_style)):
                os.system(
                    '/bin/rm '+os.path.join(self.data_directory.replace('*', ''), dataset, self.mtz_style))
                self.Logfile.insert(
    ...
    def run(self):
        ...
        for item in query:
            ...
            if os.path.isfile(os.path.join(self.initial_model_directory, xtalID, 'refine.pdb')):
                ...
                self.Logfile.insert('extracting ligand ({0!s},{1!s},{2!s},{3!s}) from refine.pdb'.format(
                    str(resname), str(chainID), str(resseq), str(altLoc)))
                XChemUtils.pdbtools(os.path.join(self.initial_model_directory, xtalID, 'refine.pdb')
                                    ).save_specific_ligands_to_pdb(resname, chainID, resseq, altLoc)
                if os.path.isfile('ligand_{0!s}_{1!s}_{2!s}_{3!s}.pdb'.format(str(resname), str(chainID), str(resseq), str(altLoc))):
                    ligand_pdb = 'ligand_{0!s}_{1!s}_{2!s}_{3!s}.pdb'.format(
                        str(resname), str(chainID), str(resseq), str(altLoc))
                else:
                    self.Logfile.insert(
                        'could not extract ligand; trying next...')
                    continue
            else:
                self.Logfile.insert('directory: '+os.path.join(
                    self.initial_model_directory, xtalID)+' -> cannot find refine.pdb; trying next')
                continue

            if os.path.isfile(os.path.join(self.initial_model_directory, xtalID, 'refine.mtz')):
                resolution = XChemUtils.mtztools(os.path.join(
                    self.initial_model_directory, xtalID, 'refine.mtz')).get_high_resolution_from_mtz()
            else:
                self.Logfile.insert('directory: '+os.path.join(
                    self.initial_model_directory, xtalID)+' -> cannot find refine.mtz; trying next')
                continue

            self.emit(QtCore.SIGNAL('update_status_bar(QString)'),
                      'eventMap -> SF for '+event_map)
            convert_event_map_to_SF(self.initial_model_directory, xtalID, event_map,
                                    ligand_pdb, self.xce_logfile, self.datasource, resolution).run()
...
class convert_event_map_to_SF:
    def __init__(self, project_directory, xtalID, event_map, ligand_pdb, xce_logfile, db_file, resolution):
        ...
        if not os.path.isfile(self.event_map):
            ...
            self.Logfile.insert(
                'cannot convert event_map to structure factors!')
            return None
        ...
        self.event = event_map[event_map.rfind(
            '/')+1:].replace('.map', '').replace('.ccp4', '')
    ...
    def run(self):
        ...
        if not os.path.isfile('{0!s}.mtz'.format(self.event)):
            ...
            self.Logfile.insert(
                'conversion successful, {0!s}.mtz exists'.format(self.event))
    ...
    def calculate_electron_density_map(self, mtzin):
        ...
        if not missing_columns:
            ...
            self.Logfile.insert(
                'cannot calculate 2fofc.map; missing map coefficients')
    ...
    def prepare_conversion_script(self):
        ...
        self.Logfile.insert(
            'preparing script for conversion of Event map to SF')
    ...
    def run_phenix_map_to_structure_factors(self):
        ...
        self.Logfile.insert('running phenix.map_to_structure_factors {0!s}_p1.map d_min={1!s} output_file_name={2!s}_tmp.mtz'.format(
            self.event, self.resolution, self.event))
        os.system('phenix.map_to_structure_factors {0!s}_p1.map d_min={1!s} output_file_name={2!s}_tmp.mtz'.format(
            self.event, self.resolution, self.event))

    def run_cinvfft(self, mtzin):
        # mtzin is usually refine.mtz
        self.Logfile.insert(
            'running cinvfft -mapin {0!s} -mtzin {1!s} -mtzout {2!s}_tmp.mtz -colout event'.format(self.event_map, mtzin, self.event))
        os.system('cinvfft -mapin {0!s} -mtzin {1!s} -mtzout {2!s}_tmp.mtz -colout event'.format(
    ...
    def update_database(self):
        sqlite = ("update panddaTable set "
                  ...
                  " where PANDDA_site_event_map is '{0!s}' ".format(
                      self.event_map)
                  )
            ...
            for files in glob.glob(xtal+'/ligand_files/*'):
                if os.path.islink(files):
                    ...
                    self.Logfile.insert(
                        'replacing symlink for {0!s} with real file'.format(files))
                    os.system(
                        'cp --remove-destination {0!s} {1!s}/ligand_files'.format(os.path.realpath(files), xtal))
            progress += progress_step
            self.emit(QtCore.SIGNAL('update_progress_bar'), progress)
...
class convert_apo_structures_to_mmcif(QtCore.QThread):
    ...
    def run(self):
        ...
        for dirs in glob.glob(os.path.join(self.panddaDir, 'processed_datasets', '*')):
            ...
            self.Logfile.insert('%s: converting %s to mmcif' %
                                (xtal, xtal+'-pandda-input.mtz'))
    ...
    def run(self):
        ...
        for item in dbEntries:
            ...
            if xtal not in dbDict:
                dbDict[xtal] = []
            dbDict[xtal].append([site, x, y, z, resname, chain, seqnum, eventMap,
                                 eventMap_mtz, initialPDB, initialMTZ, outcome, event, PanDDApath])
        ...
        for xtal in sorted(glob.glob('*')):
            if os.path.isfile(os.path.join(xtal, 'refine.pdb')):
                ligands = XChemUtils.pdbtools(os.path.join(
                    xtal, 'refine.pdb')).ligand_details_as_list()
                self.Logfile.insert(
                    '{0!s}: found file refine.pdb'.format(xtal))
                if ligands:
                    ...
                    self.Logfile.warning(
                        '{0!s}: cannot find ligand molecule in refine.pdb; skipping...'.format(xtal))
                    continue
                ...
                for n, item in enumerate(ligands):
                    ...
                    if altLocLIG.replace(' ', '') == '':
                        self.Logfile.insert(
                            xtal+': found a ligand not modelled with pandda.inspect -> {0!s} {1!s} {2!s}'.format(resnameLIG, chainLIG, seqnumLIG))
                    residue_xyz = XChemUtils.pdbtools(os.path.join(
                        xtal, 'refine.pdb')).get_center_of_gravity_of_residue_ish(item[1], item[2])
                    ...
                    if xtal in dbDict:
                        for entry in dbDict[xtal]:
                            ...
                            chainTable = entry[5]
                            seqnumTable = entry[6]
                            self.Logfile.insert('panddaTable: {0!s} {1!s} {2!s} {3!s}'.format(
                                xtal, resnameTable, chainTable, seqnumTable))
                            if resnameLIG == resnameTable and chainLIG == chainTable and seqnumLIG == seqnumTable:
                                self.Logfile.insert('{0!s}: found ligand in database -> {1!s} {2!s} {3!s}'.format(
                                    xtal, resnameTable, chainTable, seqnumTable))
                                foundLigand = True
                        if not foundLigand:
                            self.Logfile.error('{0!s}: did NOT find ligand in database -> {1!s} {2!s} {3!s}'.format(
                                xtal, resnameLIG, chainLIG, seqnumLIG))
                            ligands_not_in_panddaTable.append(
                                [resnameLIG, chainLIG, seqnumLIG, altLocLIG, occupancyLig, residue_xyz])
                    else:
                        self.Logfile.warning(
                            'ligand in PDB file, but dataset not listed in panddaTable: {0!s} -> {1!s} {2!s} {3!s}'.format(xtal, item[0], item[1], item[2]))

                for entry in ligands_not_in_panddaTable:
                    self.Logfile.error('{0!s}: refine.pdb contains a ligand that is not assigned in the panddaTable: {1!s} {2!s} {3!s} {4!s}'.format(
                ...
                for site in ligands_not_in_panddaTable:
                    ...
                    for files in glob.glob(os.path.join(self.project_directory, xtal, 'xceTmp', 'ligand_*_*.pdb')):
                        mol_xyz = XChemUtils.pdbtools(
                            files).get_center_of_gravity_of_molecule_ish()
                        ...
                        for entry in dbDict[xtal]:
                            distance = XChemUtils.misc().calculate_distance_between_coordinates(
                                mol_xyz[0], mol_xyz[1], mol_xyz[2], entry[1], entry[2], entry[3])
                            self.Logfile.insert('{0!s}: {1!s} {2!s} {3!s} <---> {4!s} {5!s} {6!s}'.format(
                                xtal, mol_xyz[0], mol_xyz[1], mol_xyz[2], entry[1], entry[2], entry[3]))
                            self.Logfile.insert(
                                '{0!s}: symm equivalent molecule: {1!s}'.format(xtal, files))
                            self.Logfile.insert(
        ...
        if self.errorDict != {}:
            self.update_errorDict(
                'General', 'The aforementioned PDB files were automatically changed by XCE!\nPlease check and refine them!!!')
...
class find_event_map_for_ligand(QtCore.QThread):
    ...
    def __init__(self, project_directory, xce_logfile, external_software):
        ...
        try:
            ...
            self.Logfile.warning(
                'cannot import gemmi; will use phenix.map_to_structure_factors instead')

    def run(self):
        self.Logfile.insert(
            '======== checking ligand CC in event maps ========')
            ...
            if os.path.isfile(os.path.join(dirs, 'refine.pdb')) and \
               os.path.isfile(os.path.join(dirs, 'refine.mtz')):
                ...
                ligList = XChemUtils.pdbtools(
                    'refine.pdb').save_residues_with_resname(dirs, 'LIG')
                self.Logfile.insert(
                    '%s: found %s ligands of type LIG in refine.pdb' % (xtal, str(len(ligList))))
                ...
                for maps in glob.glob(os.path.join(dirs, '*event*.native.ccp4')):
                    if self.external_software['gemmi']:
                        ...
                        self.convert_map_to_sf(
                            maps.replace('.ccp4', '.P1.ccp4'), reso)
    ...
    def convert_map_to_sf_with_gemmi(self, emap, p):
        self.Logfile.insert(
            'converting ccp4 map to mtz with gemmi map2sf: %s' % emap)
        if os.path.isfile(emap.replace('.ccp4', '.mtz')):
            self.Logfile.warning('mtz file of event map exists; skipping...')
            return
        cmd = 'gemmi map2sf %s %s FWT PHWT --dmin=%s' % (
            emap, emap.replace('.ccp4', '.mtz'), p.resolution)
