...
def zenodo_upload_part_two():
    ...
    instruction = (
        'You might have noticed that once you started uploading files, the address bar in your browser will change to something like this:')
...
def zenodo_upload_part_three():
    ...
    instruction = (
        'Please enter the upload ID, i.e. the number at the end of the line, into the field below')
...
def zenodo_upload_part_four(html_export_directory):
    ...
    instruction = ('Upload ALL html files in {0!s} to ZENODO and publish the page.'.format(
