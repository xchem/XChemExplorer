...
def create_pdb_cif_png_files():
    tip = (
        "uses ACEDRG to create pdb,cif and png files for all compounds\n"
...
def check_status_create_pdb_cif_png_files():
    tip = (
        "the jobs are sent to a computer cluster if available\n"
...
def html_summary_introduction():
    ...
    text = (
        "This section describes how to generate an HTML summary page of the current project. \n"
...
def html_export_directory_background():
    ...
    text = (
        "The HTML export directory will contain all files necessary to generate the summary file.\n"
...
def html_export_step():
    ...
    text = (
        "This step will create an index.html file and it will copy pdb, mtz and cif files\n"
...
def prepare_ICB_files():
    ...
    instruction = (
        "- Open file browser and navigate to HTML export directory.\n"
...
def zenodo_upload_start(html_export_directory):
    ...
    instruction = (
        "This will copy all files required for ZENODO upload to\n"
...
def zenodo_upload_part_one(html_export_directory):
    ...
    instruction = (
        "- Register with ZENODO (www.zenodo.org)\n"
        "- Select Upload -> New Upload\n"
        "- upload all files from %s/zenodo, BUT NOT any of the .html files!!!"
...
def zenodo_upload_part_four(html_export_directory):
    ...
    instruction = (
        "Upload ALL html files in {0!s} to ZENODO and publish the page.".format(
            os.path.join(html_export_directory, "zenodo")
        )
    )
...
def deposition_pandda_site_not_ready(xtal):
    msg = (
        "Please make sure that all placed ligands in "
        + xtal
...
def deposition_introduction_link():
    lnk = "<a href=\"https://openlabnotebooks.org/update-on-batch-deposition-of-xchem-structures\">'https://openlabnotebooks.org/update-on-batch-deposition-of-xchem-structures'</a>"
    return lnk
...
def pdb_group_deposition_instruction_one():
    msg = "3. Go to the group deposition website, create a session and upload the ligand-bound.tar.bz2 file from the group deposition directory."
    return msg


def pdb_group_deposition_link():
    lnk = "<a href=\"https://deposit-group-1.rcsb.rutgers.edu/groupdeposit\">'https://deposit-group-1.rcsb.rutgers.edu/groupdeposit'</a>"
    return lnk
...
def deposition_ground_state_preparation_step_eight_text():
    msg = "6. Go to the group deposition website, create a session and upload the ligand-bound.tar.bz2 file from the group deposition directory."
    return msg
...
def deposition_bounnd_state_preparation_ignore_event_map():
    msg = "do NOT include PanDDA event maps (ONLY USE IN CASE IF DATA WERE NOT ANALYSED WITH PANDDA!)"
    return msg
