...
def get_dewar_configuration(beamline_directory):
    ...
    for xml in glob.glob(os.path.join(beamline_directory, 'xml', 'exptTableParams-*.xml')):
        ...
        dewar_sample_configuration_dict[str(
            container_reference)+'-'+str(sample_location)] = prefix
...
def print_acedrg_status(xce_logfile, xtal_db_dict):
    ...
    for xtal in xtal_db_dict:
        ...
        if 'pending' in status:
            ...
            unknown += 1
    Logfile.insert(
        'restraint generation pending: ...... {0!s}'.format(str(pending)))
    Logfile.insert(
        'restraint generation started: ...... {0!s}'.format(str(started)))
    Logfile.insert(
        'restraint generation running: ...... {0!s}'.format(str(running)))
    Logfile.insert(
        'missing smiles string: ............. {0!s}'.format(str(missing_smiles)))
    Logfile.insert(
        'restraint generation failed: ....... {0!s}'.format(str(failed)))
    Logfile.insert(
        'restraints successfully created: ... {0!s}'.format(str(success)))
    Logfile.insert(
        'unknown status: .................... {0!s}'.format(str(unknown)))
...
#                relative_path_to_mtzfile='./'+path_to_mtzfile.replace(initial_model_directory,'')
                relative_path_to_mtzfile = './' + \
                    path_to_mtzfile.replace(os.path.join(
                        initial_model_directory, sample), '')
                if relative_path_to_mtzfile.startswith('.//'):
                    relative_path_to_mtzfile = relative_path_to_mtzfile.replace(
                        './/', './')
                relative_path_to_logfile = './' + \
                    path_to_logfile.replace(os.path.join(
                        initial_model_directory, sample), '')
                ...
                if os.path.isdir(os.path.join(initial_model_directory, sample, 'autoprocessing', visit+'-'+run+autoproc)):
                    ...
                    Logfile.insert('changing directory to: ' +
...
#                    os.symlink(os.path.join(path_to_mtzfile,mtz_filename),sample+'.mtz')
                    Logfile.insert('setting relative symlink: '+os.path.join(
                        relative_path_to_logfile, log_filename)+' -> '+sample+'.log')
                    os.symlink(os.path.join(relative_path_to_logfile,
                                            log_filename), sample+'.log')
                    Logfile.insert('setting relative symlink: '+os.path.join(
                        relative_path_to_mtzfile, mtz_filename)+' -> '+sample+'.mtz')
                    os.symlink(os.path.join(relative_path_to_mtzfile,
                ...
                if found_barcode_entry:
                    if 'Snapshots will be saved' in line:
                        sampleID = line.split()[len(
                            line.split())-1].split('/')[-1]
                        out_dict[sampleID] = barcode
                        Logfile.insert('found: sample={0!s}, barcode={1!s}, file={2!s}'.format(
...
def linkAutoProcessingResult(xtal, dbDict, projectDir, xce_logfile):
    ...
    Logfile.insert(xtal+': looking for '+os.path.join('autoprocessing',
                                                      visit + '-' + run + autoproc + procCode, mtzfile))
    if os.path.isfile(os.path.join('autoprocessing', visit + '-' + run + autoproc + procCode, mtzfile)):
        os.symlink(os.path.join('autoprocessing', visit + '-' +
                                run + autoproc + procCode, mtzfile), xtal + '.mtz')
        Logfile.insert(
...
def getVisitAndBeamline(visitDirectory):
    ...
    if not visitDirectory.startswith('/dls'):
        ...
        if visitDirectory.split('/')[len(visitDirectory.split('/')) - 2] == 'processed':
            visit = visitDirectory.split(
                '/')[len(visitDirectory.split('/')) - 3]
        else:
...
def backup_soakDB(database, xce_logfile):
    Logfile = XChemLog.updateLog(xce_logfile)
    Logfile.insert('backing up soakDB: ' + database +
                   str(datetime.now()).replace(' ', '_'))
    os.system('/bin/cp %s %s' % (database, database+'.' +
...
class find_diffraction_image_directory(QtCore.QThread):
    def __init__(self, diffraction_data_directory):
        ...
        self.data_dict = {}
        self.diffraction_image_extension = [
            '.img', '.cbf', '.mccd', '.mar2560', '.mar2300']
#        self.datasetID_to_sampleID_conversion='*'
    ...
    def run(self):
        ...
        if len(glob.glob(os.path.join(self.diffraction_data_directory, '*'))) != 0:
            progress_step = 100 / \
                float(len(glob.glob(os.path.join(self.diffraction_data_directory, '*'))))
        else:
            progress_step = 100
        progress = 0
                ...
                for n, image_file in enumerate(glob.glob(os.path.join(root, '*'))):
                    ...
                    if n > 20 and file_extension in self.diffraction_image_extension:
                        if os.path.join(self.diffraction_data_directory, root) not in self.data_dict[xtal][0]:
                            self.data_dict[xtal][0].append(os.path.join(
                                self.diffraction_data_directory, root))
#                        break
        ...
        self.emit(QtCore.SIGNAL(
            'update_datasets_reprocess_table'), self.data_dict)
