...
#        self.hbox_select_samples.add(self.cb_select_sites)
        ...
        self.table_right.attach(ligandIDLabel_frame,
                                0, 1, 0, 1)
        self.table_right.attach(
            ligand_occupancyLabel_frame,           0, 1, 1, 2)
        self.table_right.attach(
            ligand_BaverageLabel_frame,            0, 1, 2, 3)
        self.table_right.attach(
            ligand_BratioSurroundingsLabel_frame,  0, 1, 3, 4)
        self.table_right.attach(
            ligand_RSCCLabel_frame,                0, 1, 4, 5)
        self.table_right.attach(
            ligand_rmsdLabel_frame,                0, 1, 5, 6)
        self.table_right.attach(
            ligand_RSRLabel_frame,                 0, 1, 6, 7)
        self.table_right.attach(
            ligand_RSZDLabel_frame,                0, 1, 7, 8)
        self.table_right.attach(ligandIDBox_frame,
                                1, 2, 0, 1)
        self.table_right.attach(
            ligand_occupancyBox_frame,             1, 2, 1, 2)
        self.table_right.attach(
            ligand_BaverageBox_frame,              1, 2, 2, 3)
        self.table_right.attach(
            ligand_BratioSurroundingsBox_frame,    1, 2, 3, 4)
        self.table_right.attach(
            ligand_RSCCBox_frame,                  1, 2, 4, 5)
        self.table_right.attach(
            ligand_rmsdBox_frame,                  1, 2, 5, 6)
        self.table_right.attach(ligand_RSRBox_frame,
                                1, 2, 6, 7)
        self.table_right.attach(
            ligand_RSZDBox_frame,                  1, 2, 7, 8)
        ...
        pic = gtk.gdk.pixbuf_new_from_file(os.path.join(
            os.getenv('XChemExplorer_DIR'), 'image', 'NO_COMPOUND_IMAGE_AVAILABLE.png'))
...
#        self.hbox_for_info_graphics.add(self.canvas)
        ...
        spider_plot_pic = gtk.gdk.pixbuf_new_from_file(os.path.join(
        ...
        ligand_inspect_confidence_label_frame.add(
            ligand_inspect_confidence_label)
        ligand_inspect_interesting_label_frame = gtk.Frame()
        ligand_inspect_interesting_label = gtk.Label('Interesting')
        ligand_inspect_interesting_label_frame.add(
            ligand_inspect_interesting_label)
        ...
        ligand_inspect_confidence_value_frame.add(
            self.ligand_inspect_confidence_value)
        ...
        ligand_inspect_comment_value_frame.add(
        ...
        pandda_inspect_comments_table.attach(
            ligand_site_name_label_frame,              0, 1, 0, 1)
        pandda_inspect_comments_table.attach(
            ligand_site_name_value_frame,              1, 2, 0, 1)
        pandda_inspect_comments_table.attach(
...
#        self.DEPOSITbutton = gtk.Button(label="prepare for deposition")
    ...
    def RefreshSiteData(self):
        ...
        try:
            self.ligand_BratioSurroundingsValue.set_label(str(round(
                float(self.spider_plot_data['PANDDA_site_B_ratio_residue_surroundings']), 2)))
        except ValueError:
...
#        self.canvas.show()
        ...
        if os.path.isfile(os.path.join(self.project_directory, self.xtalID, self.compoundID+'.pdb')):
            imol = coot.handle_read_draw_molecule_with_recentre(os.path.join(
                self.project_directory, self.xtalID, self.compoundID+'.pdb'), 0)
...
#                        os.symlink('dimple.mtz',self.mtz_style)
        ...
        try:
            self.RRfreeBox.modify_bg(gtk.STATE_NORMAL, gtk.gdk.color_parse(
        ...
        self.ResolutionValue.set_label(
            self.QualityIndicators['RefinementResolution'])
        try:
            self.ResolutionBox.modify_bg(gtk.STATE_NORMAL, gtk.gdk.color_parse(
                self.QualityIndicators['RefinementResolutionTL']))
        except ValueError:
            pass
        ...
        self.rmsdBondsValue.set_label(
            self.QualityIndicators['RefinementRmsdBonds'])
        try:
            self.rmsdBondsBox.modify_bg(gtk.STATE_NORMAL, gtk.gdk.color_parse(
                self.QualityIndicators['RefinementRmsdBondsTL']))
        except ValueError:
            pass
        self.rmsdAnglesValue.set_label(
            self.QualityIndicators['RefinementRmsdAngles'])
        try:
            self.rmsdAnglesBox.modify_bg(gtk.STATE_NORMAL, gtk.gdk.color_parse(
                self.QualityIndicators['RefinementRmsdAnglesTL']))
        except ValueError:
            pass
    ...
    def REFINE(self, widget):
        ...
        for item in coot_utils_XChem.molecule_number_list():
            if coot.molecule_name(item).endswith(self.pdb_style):
                coot.write_pdb_file(item, os.path.join(
                    self.project_directory, self.xtalID, 'Refine_'+str(self.Serial), 'in.pdb'))
                break
