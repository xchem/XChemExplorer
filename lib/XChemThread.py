...
class synchronise_db_and_filesystem(QtCore.QThread):
    ...
    def run(self):
        ...
        self.Logfile.insert('current project directory: ' +
        ...
        if self.mode != 'project_directory':
            ...
            self.Logfile.insert('found '+str(len(glob.glob(os.path.join(
                self.initial_model_directory, '*'))))+' samples in project directory')
        ...
        for xtal in self.xtal_list:
            self.Logfile.insert('directory name: '+xtal +
                                ' = sampleID in database')
            os.chdir(os.path.join(self.initial_model_directory, xtal))
            if xtal not in self.all_samples_in_datasource:
                self.Logfile.insert(
                    'sampleID not found in database: inserting '+xtal)
...
#        self.sync_pandda_table()
    ...
    def change_absolute_to_relative_links(self, target, filename):
        ...
        self.Logfile.insert(
            '{0!s} -> {1!s}'.format(os.readlink(filename), target))
                ...
                try:
                    ...
                    if link.startswith('/'):
                        if os.path.isfile(link.replace(link[:link.find(xtal)], self.initial_model_directory+'/')):
                            ...
                            found_file = True
                            self.change_absolute_to_relative_links(
                                target, filename)
                            ...
                            self.Logfile.insert(
                                'removing broken link: '+filename)
                            os.system(
                                '/bin/rm {0!s} 2> /dev/null'.format(filename))
                    else:
        ...
        if os.path.isfile('refine.mtz'):
            if os.path.isfile(xtal+'.free.mtz'):
                ...
                if os.path.isfile(xtal+'.mtz'):
                    ...
                    CC, errorMessage = freeMTZ.calculate_correlaton_between_intensities_in_mtzfiles(
                        xtal+'.mtz')
                    ...
                    if errorMessage != '':
                        self.Logfile.insert(
                            'pointless failed with the following error: %s' % errorMessage)
                        ...
                        if float(CC) < 0.999:
                            ...
                            if os.path.isfile('dimple/dimple_rerun_on_selected_file/dimple/dimple.log'):
                                ...
                                for line in open('dimple/dimple_rerun_on_selected_file/dimple/dimple.log'):
                                    if foundLine:
                                        mtzin = line.replace(' ', '').replace(
                                            '\n', '').replace('\r', '')
                                        self.Logfile.insert(
                                            '%s was used for inital map calculation' % mtzin)
                                        break
                                ...
                                if os.path.isfile(mtzin):
                                    ...
                                    self.Logfile.insert(
                                        '%s: removing links for %s.mtz/%s.log' % (xtal, xtal, xtal))
                                    os.system(
                                        '/bin/rm %s.mtz 2> /dev/null' % xtal)
                                    os.system(
                                        '/bin/rm %s.log 2> /dev/null' % xtal)
                                    self.Logfile.insert('linking %s to %s.mtz' % (
                                        os.path.relpath(mtzin), xtal))
                                    os.symlink(os.path.relpath(
                                        mtzin), xtal+'.mtz')
                                    for logfile in glob.glob(os.path.join(mtzin[:mtzin.rfind('/')], '*log')):
                                        self.Logfile.insert('linking %s to %s.log' % (
...
#            db_dict['DataProcessingPathToLogfile']=os.path.realpath(xtal+'.log').replace(os.getcwd()+'/','')
            db_dict['DataProcessingPathToLogfile'] = os.path.realpath(
                xtal+'.log')
...
#        if found_mtzfile:
        if os.path.isfile(xtal+'.mtz'):
            db_dict['DataProcessingPathToMTZfile'] = os.path.realpath(
                xtal+'.mtz')
#            db_dict['DataProcessingPathToMTZfile']=os.path.realpath(xtal+'.mtz').replace(os.getcwd()+'/','')
            ...
            if not found_logfile:
                mtz_info = mtztools(
                    xtal+'.mtz').get_information_for_datasource()
...
#            db_dict['RefinementMTZfree']=os.path.realpath(xtal+'.free.mtz').replace(os.getcwd()+'/','')
            ...
            if os.path.isfile(os.path.join(dimple_path, 'prepared2.mtz')):
                os.symlink(os.path.relpath(os.path.join(
                    dimple_path, 'prepared2.mtz')), xtal+'.free.mtz')
#                db_dict['RefinementMTZfree']=os.path.realpath(xtal+'.free.mtz').replace(os.getcwd()+'/','')
                db_dict['RefinementMTZfree'] = os.path.realpath(
                    xtal+'.free.mtz')
#                db_dict['RefinementMTZfree']=xtal+'.free.mtz'
            elif os.path.isfile(os.path.join(dimple_path, 'prepared.mtz')):
                os.symlink(os.path.relpath(os.path.join(
                    dimple_path, 'prepared.mtz')), xtal+'.free.mtz')
                db_dict['RefinementMTZfree'] = os.path.realpath(
                    xtal+'.free.mtz')
#                db_dict['RefinementMTZfree']=os.path.realpath(xtal+'.free.mtz').replace(os.getcwd()+'/','')
#               db_dict['RefinementMTZfree']=xtal+'.free.mtz'
                ...
                os.symlink(os.path.relpath(os.path.join(
                    dimple_path, 'free.mtz')), xtal+'.free.mtz')
                db_dict['RefinementMTZfree'] = os.path.realpath(
                    xtal+'.free.mtz')
    ...
    def sync_compound_information(self, db_dict):
        ...
        if not os.path.isfile(compoundID+'.pdb') or os.path.getsize(compoundID+'.pdb') < 20:
            ...
            os.system(
                '/bin/rm compound/{0!s}.pdb 2> /dev/null'.format(compoundID))
        ...
        if not os.path.isfile(compoundID+'.png') or os.path.getsize(compoundID+'.png') < 20:
            ...
            os.system(
                '/bin/rm compound/{0!s}.png 2> /dev/null'.format(compoundID))
...
#                db_pandda_dict['PANDDApath']=self.panddas_directory
                if str(apoStructures) == 'None' or apoStructures == '':
                    if panddaPATH != 'None' or panddaPATH != '':
                        ...
                        db_pandda_dict['ApoStructures'] = self.find_apo_structures_for_PanDDA(
                            panddaPATH)
                    else:
                        self.Logfile.insert(
...
#                        db_pandda_dict['PANDDA_site_event_map']=os.path.realpath(event_map).replace(os.getcwd()+'/','')
                ...
                if os.path.isfile(os.path.join(self.initial_model_directory, xtal, 'refine.pdb')):
                    ligands_in_file = pdbtools(os.path.join(
                        self.initial_model_directory, xtal, 'refine.pdb')).find_xce_ligand_details()
                    if not ligands_in_file:
                        self.Logfile.warning(
                            '{0!s}: could not find any ligands in refine.pdb'.format(xtal))
                        continue
                    else:
                        self.Logfile.insert('{0!s}: found the following ligands in refine.pdb: {1!s}'.format(
                    ...
                    for ligand in ligands_in_file:
                        ...
                        residue_number = ligand[2]
                        residue_altLoc = ligand[3]
                        residue_xyz = pdbtools(os.path.join(self.initial_model_directory, xtal, 'refine.pdb')
                                               ).get_center_of_gravity_of_residue_ish(residue_chain, residue_number)
                        distance = misc().calculate_distance_between_coordinates(
                            residue_xyz[0], residue_xyz[1], residue_xyz[2], event_x, event_y, event_z)
                        distanceList.append(
                            [distance, residue_name, residue_chain, residue_number, residue_altLoc])
                        self.Logfile.insert('{0!s}: calculating distance between event and ligand ({1!s} {2!s} {3!s}): {4!s}'.format(
                    ...
                    self.Logfile.insert('{0!s}: ligand with the shorted distance ({1!s}A) to the current event (id: {2!s}): {3!s} {4!s} {5!s} {6!s}'.format(
                        xtal, str(distance), event_index, residue_name, residue_chain, residue_number, residue_altLoc))
                    db_pandda_dict['PANDDA_site_ligand_resname'] = residue_name
                    db_pandda_dict['PANDDA_site_ligand_chain'] = residue_chain
                    db_pandda_dict['PANDDA_site_ligand_sequence_number'] = residue_number
                    db_pandda_dict['PANDDA_site_ligand_altLoc'] = residue_altLoc
                    db_pandda_dict['PANDDA_site_ligand_placed'] = 'True'
                    db_pandda_dict['PANDDA_site_ligand_id'] = residue_name + \
                        '-'+residue_chain+'-'+residue_number

                    if xtal+'/Refine_' in os.path.realpath(os.path.join(self.initial_model_directory, xtal, 'refine.pdb')):
                        tmp = os.path.realpath(os.path.join(
                            self.initial_model_directory, xtal, 'refine.pdb'))
                        spider_plot = os.path.join(tmp[:tmp.rfind(
                            '/')], 'residue_plots', residue_chain+'-'+residue_number+'.png').replace(' ', '')
                        if os.path.isfile(spider_plot):
                            db_pandda_dict['PANDDA_site_spider_plot'] = os.path.realpath(
                                spider_plot)
                        if os.path.isfile(os.path.join(tmp[:tmp.rfind('/')], 'residue_scores.csv')):
                            with open(os.path.join(tmp[:tmp.rfind('/')], 'residue_scores.csv'), 'rb') as csv_import:
                                csv_dict = csv.DictReader(csv_import)
...
#                    break
                ...
                if db_pandda_dict != {}:
                    ...
                    self.Logfile.insert('updating panddaTable for xtal: {0!s}, site: {1!s}, event: {2!s}'.format(
                        xtal, site_index, event_index))
                    self.Logfile.insert(
                        '-> panddaDict: ' + str(db_pandda_dict))
...
class create_png_and_cif_of_compound(QtCore.QThread):
    def __init__(self, external_software, initial_model_directory, compound_list, database_directory, data_source_file, todo, ccp4_scratch_directory, xce_logfile, max_queue_jobs, restraints_program):
        ...
        self.db = XChemDB.data_source(os.path.join(
        ...
        for item in self.compound_list:
            ...
            if self.todo == 'ALL' or self.todo == 'SELECTED':
                ...
                if os.path.isfile(os.path.join(self.initial_model_directory, sampleID, compoundID.replace(' ', '')+'.pdb')):
                    os.system('/bin/rm '+os.path.join(self.initial_model_directory,
                                                      sampleID, compoundID.replace(' ', '')+'.pdb'))
...
#                    os.system('/bin/rm '+os.path.join(self.initial_model_directory,sampleID,compoundID.replace(' ','')+'.cif'))
                if os.path.isfile(os.path.join(self.initial_model_directory, sampleID, compoundID.replace(' ', '')+'.png')):
                    os.system('/bin/rm '+os.path.join(self.initial_model_directory,
                                                      sampleID, compoundID.replace(' ', '')+'.png'))
                if os.path.isdir(os.path.join(self.initial_model_directory, sampleID, 'compound')):
                    os.system(
                        '/bin/rm -fr '+os.path.join(self.initial_model_directory, sampleID, 'compound'))
                    db_dict = {'RefinementCIFStatus': 'pending'}
                    self.Logfile.insert(
                        '{0!s}: removed compound directory and all its contents'.format(sampleID))
                    self.Logfile.insert(
                        '{0!s}: setting RefinementCIFStatus flag to started'.format(sampleID))
        ...
        self.Logfile.insert('created input scripts for '+str(counter) +
                            ' ACEDRG jobs in '+self.ccp4_scratch_directory)
        os.chdir(self.ccp4_scratch_directory)
        ...
        if counter > 1:
            ...
            if os.path.isdir('/dls'):
                if self.external_software['qsub_array']:
                    ...
                    self.Logfile.insert('         qsub -P labxchem -q medium.q -t 1:{0!s} -tc {1!s} {2!s}_master.sh'.format(
                        str(counter), self.max_queue_jobs, self.restraints_program))
                    os.system('qsub -P labxchem -q medium.q -t 1:{0!s} -tc {1!s} -N {2!s} {3!s}_master.sh'.format(
                        str(counter), self.max_queue_jobs, self.restraints_program, self.restraints_program))
                else:
                    self.Logfile.insert(
                ...
                for i in range(counter):
                    self.Logfile.insert('qsub -q medium.q -N {0!s} xce_{1!s}_{2!s}.sh'.format(
                        self.restraints_program, self.restraints_program, (str(i+1))))
                    os.system('qsub -N {0!s} xce_{1!s}_{2!s}.sh'.format(
                        self.restraints_program, self.restraints_program, (str(i+1))))
            else:
                self.Logfile.insert('running %s consecutive %s jobs on your local machine' % (
                    str(counter), self.restraints_program))
                for i in range(counter):
                    self.Logfile.insert('starting xce_{0!s}_{1!s}.sh'.format(
                        self.restraints_program, (str(i+1))))
                    os.system(
                        './xce_{0!s}_{1!s}.sh'.format(self.restraints_program, (str(i+1))))
...
class fit_ligands(QtCore.QThread):
    ...
    def run(self):
        ...
        for item in self.compound_list:
            ...
            if os.path.isdir(os.path.join(self.initial_model_directory, sampleID, 'autofit_ligand')):
                os.system(
                    '/bin/rm -fr ' + os.path.join(self.initial_model_directory, sampleID, 'autofit_ligand'))
            os.mkdir(os.path.join(self.initial_model_directory,
                                  sampleID, 'autofit_ligand'))
    ...
    def run_script(self):
        ...
        self.Logfile.insert('created input scripts for ' +
                            str(self.n) + ' in ' + self.ccp4_scratch_directory)
        os.chdir(self.ccp4_scratch_directory)
        if os.path.isdir('/dls'):
            if self.external_software['qsub_array']:
                Cmds = (
...
class merge_cif_files(QtCore.QThread):
    ...
    def run(self):
        ...
        for item in self.compound_list:
            ...
            if os.path.isfile(os.path.join(self.initial_model_directory, sampleID, 'compound', compoundID + '.cif')):
                self.Logfile.insert(
                    '%s: found %s.cif file in compound sub-directory' % (sampleID, compoundID))
            else:
                self.Logfile.error(
            ...
            if os.path.isfile(os.path.join(self.initial_model_directory, sampleID, compoundID+'.cif')):
                self.Logfile.warning(
                    '%s: removing symbolic link to (or file) %s.cif from sample directory' % (sampleID, compoundID))
            ...
            if self.todo == 'merge':
                ...
                self.Logfile.insert(
                    '%s: restoring symbolic link -> ln -s compound/%s.cif .' % (sampleID, compoundID))
    ...
    def run_libcheck(self, sampleID, compoundID):
        ...
        if os.path.isfile(compoundID+'.cif.lib'):
            self.Logfile.insert(
                '%s: merged CIF file successfully created' % sampleID)
            os.system('/bin/mv %s %s' %
                      (compoundID+'.cif.lib', compoundID+'.cif'))
        else:
            self.Logfile.error(
                '%s: could not create merged CIF file' % sampleID)
            self.Logfile.warning(
                '%s: will re-create symbolic links to original restraints file' % sampleID)
            os.system('ln -s compound/%s.cif .' % compoundID)
...
# pipedream -d pipedreamDir -xyzin $reference.pdb -hklref $referenceDataset.mtz -hklin $mtzFileUnique.mtz -rhofit $inhib.cif  -target $reference.pdb -keepwater -nthreads 12 -postquick -allclusters |tee -a pipedream.log
    ...
    def prepare_dimple_shell_script(self, xtal, visit_run_autoproc, mtzin, ref_pdb, ref_mtz, ref_cif):
        ...
        if os.path.isfile(ref_mtz):
            ...
            if 'FreeR_flag' not in mtz_column_dict['RFREE']:
                self.Logfile.insert(
                    'cannot find FreeR_flag in reference mtz file: {0!s} -> ignoring reference mtzfile!!!'.format(ref_mtz))
                ref_mtz = ''
                if mtz_column_dict['RFREE']:
                    self.Logfile.insert('found Rfree set with other column name though: {0!s}'.format(
        ...
        if self.dimple_twin_mode:
            ...
            os.mkdir(os.path.join(
                self.initial_model_directory, xtal, 'dimple_twin'))
        ...
        if mtzFile.space_group_info().symbol_and_number() == 'R 3 :H (No. 146)':
            ...
            symNoAbsence = str([x[0] for x in str(mtzFile.space_group_info().symbol_and_number().split(
                '(')[0]).split()]).replace('[', '').replace(']', '').replace("'", "").replace(',', '').replace(' ', '')
        ...
        os.chdir(self.ccp4_scratch_directory)
        f = open('xce_{0!s}{1!s}_{2!s}.sh'.format(
            self.pipeline, twin, str(self.n)), 'w')
        f.write(Cmds)
        f.close()
        os.system(
    ...
    def run_script(self, twin):
        ...
        if os.path.isdir('/dls'):
            if self.external_software['qsub_array']:
                ...
                f = open('{0!s}{1!s}_master.sh'.format(
                    self.pipeline, twin), 'w')
                f.write(Cmds)
                f.close()
                self.Logfile.insert(
                    'submitting array job with maximal 100 jobs running on cluster')
                ...
                os.system('qsub -P labxchem -q medium.q -t 1:{0!s} -tc {1!s} {2!s}{3!s}_master.sh'.format(
            ...
            for i in range(1, self.n):
                self.Logfile.insert(
                    'qsub -q medium.q xce_{0!s}{1!s}_{2!s}.sh'.format(self.pipeline, twin, str(i)))
                os.system(
                    'qsub -q medium.q xce_{0!s}{1!s}_{2!s}.sh'.format(self.pipeline, twin, str(i)))
        else:
            self.Logfile.insert('running {0!s} consecutive {1!s} jobs on your local machine'.format(
                str(self.n-1), self.pipeline))
            for i in range(1, self.n):
                self.Logfile.insert('starting xce_{0!s}{1!s}_{2!s}.sh'.format(
...
class remove_selected_dimple_files(QtCore.QThread):
    ...
    def run(self):
        ...
        for n, xtal in enumerate(self.sample_list):
            ...
            if not os.path.isdir(os.path.join(self.initial_model_directory, xtal)):
                self.Logfile.insert(
                    '{0!s}: directory does not exist'.format(xtal))
                continue
            os.chdir(os.path.join(self.initial_model_directory, xtal))
            ...
            if self.pipeline == 'dimple':
                if os.path.isfile('init.pdb'):
                    if 'dimple' in os.path.realpath('init.pdb'):
                        self.Logfile.warning(
                            '{0!s}: init.pdb & init.mtz is linked to dimple outcome'.format(xtal))
                        self.Logfile.warning(
                ...
                if os.path.isfile('init.pdb'):
                    if 'dimple' in os.path.realpath('init.pdb'):
                        self.Logfile.warning(
                            '{0!s}: init.pdb & init.mtz is linked to pipedream outcome'.format(xtal))
                        self.Logfile.warning(
                            '{0!s}: removing init.pdb & init.mtz & (2)fofc maps'.format(xtal))
                        db_dict = self.remove_init(db_dict)
                else:
                    db_dict = self.remove_init(db_dict)
                self.Logfile.warning(
                    '{0!s}: removing pipedream folder & pipedream.pdb/pipedream.mtz'.format(xtal))
                ...
                if os.path.isfile('init.pdb'):
                    if 'dimple' in os.path.realpath('init.pdb'):
                        self.Logfile.warning(
                            '{0!s}: init.pdb & init.mtz is linked to phenix.ligand_pipeline outcome'.format(xtal))
                        self.Logfile.warning(
                            '{0!s}: removing init.pdb & init.mtz & (2)fofc maps'.format(xtal))
                        db_dict = self.remove_init(db_dict)
                else:
                    db_dict = self.remove_init(db_dict)
                self.Logfile.warning(
                    '{0!s}: removing phenix.ligand_pipeline folder & phenix.ligand_pipeline.pdb/phenix.ligand_pipeline.mtz'.format(xtal))
                os.system(
                    '/bin/rm phenix.ligand_pipeline_run_in_progress 2> /dev/null')
...
class set_results_from_selected_pipeline(QtCore.QThread):
    ...
    def run(self):
        ...
        for n, xtal in enumerate(self.sample_list):
            ...
            if os.path.isfile('init.pdb'):
                self.Logfile.warning(
                    '{0!s}: init.pdb & init.mtz exist'.format(xtal))
                self.Logfile.warning(
                    '{0!s}: removing init.pdb & init.mtz & (2)fofc maps'.format(xtal))
                os.system('/bin/rm init.pdb')
                os.system('/bin/rm init.mtz')
            ...
            if self.pipeline == 'dimple':
                if os.path.isfile('dimple.pdb'):
                    self.Logfile.insert(
                        '%s: selecting output from dimple' % xtal)
                ...
                if os.path.isfile('pipedream.pdb'):
                    self.Logfile.insert(
                        '%s: selecting output from pipedream' % xtal)
...
class start_COOT(QtCore.QThread):
    ...
    def run(self):
        ...
        pickle.dump(self.settings, open(os.path.join(
            os.getenv('HOME'), '.xce_settings.pkl'), 'wb'))
        os.system('cd {0!s}\ncoot --no-guano --no-state-script --script {1!s}'.format(
            os.getenv('HOME'), os.path.join(os.getenv('XChemExplorer_DIR'), 'lib', self.pylib)))
...
class read_pinIDs_from_gda_logs(QtCore.QThread):
    def __init__(self,
        ...
        self.allSamples = self.db.collected_xtals_during_visit_for_scoring(
            visit)
    ...
    def run(self):
        if self.gzipped_logs_parsed:
            self.Logfile.insert(
                "parsed gzipped gda logfiles before, won't do again...")
        self.Logfile.insert("will start parsing of current gda logfile at line {0!s}".format(
        ...
        self.emit(QtCore.SIGNAL(
            'update_gdaLog_parsing_instructions_and_score'), self.gdaLogInstructions)
        self.emit(QtCore.SIGNAL("finished()"))
...
#        self.allSamples = self.db.collected_xtals_during_visit_for_scoring(visit,rescore)
    ...
    def run(self):
        ...
        for sample in sorted(self.allSamples):
            if self.db.autoprocessing_result_user_assigned(sample) and not self.rescore:
                if os.path.isfile(os.path.join(self.projectDir, sample, sample+'.mtz')):
                    ...
                    self.Logfile.warning(
                        '{0!s}: user has manually selected auto-processing result before, but {1!s}.mtz does not exist'.format(sample, sample))
                    self.Logfile.insert(
                        '%s: selecting autoprocessing result' % sample)
            elif self.rescore:
                self.Logfile.warning(
                    '{0!s}: rescore selected -> might overwrite user selection!'.format(sample))
            else:
                self.Logfile.insert(
                    '%s: selecting autoprocessing result' % sample)
            dbList = self.db.all_autoprocessing_results_for_xtal_as_dict(
                sample)
            self.Logfile.insert(
                '%s: found %s different autoprocessing results' % (sample, str(len(dbList))))
            ...
            dbDict['DataCollectionOutcome'] = self.determine_processing_outcome(
                dbDict)
        ...
        self.Logfile.insert(
            '====== finished scoring data processing results ======')
    ...
    def report_forward_carried_pipelines(self, dbListOut, dbList):
        ...
        self.Logfile.insert('{0:30} {1:10} {2:10} {3:10}'.format(
    ...
    def checkExistingFiles(self, dbList, xtal):
        ...
        self.Logfile.insert(xtal + ': changing directory ' +
        ...
        for resultDict in dbList:
            try:
                ...
                mtzfile = os.path.join(
                    'autoprocessing', visit + '-' + run + autoproc + procCode, mtzfileName)
                logfile = os.path.join(
                    'autoprocessing', visit + '-' + run + autoproc + procCode, logfileName)
                ...
                if isinstance(float(resultDict['DataProcessingUnitCellVolume']), float):
                    ...
                    for reference_file in self.reference_file_list:
                        if not reference_file[4] == 0:
                            ...
                            self.Logfile.insert(
                                'unitcell volume dataset:  ' + str(resultDict['DataProcessingUnitCellVolume']))
                            unitcell_difference = round((math.fabs(
                                reference_file[4]-float(resultDict['DataProcessingUnitCellVolume']))/reference_file[4])*100, 1)
    ...
    def selectResultsWithAcceptableLowResoRmerge(self, dbList):
        self.Logfile.insert('checking if MTZ files have acceptable low resolution Rmerge values (currently set to %s)' % str(
    ...
    def selectSpecificPipelineOnly(self, dbList):
        tmp = []
        self.Logfile.insert(
            'selecting datasets by auto-processing pipeline: ' + self.selection_mechanism)
        for resultDict in dbList:
...
#        self.target = target
        ...
        self.visit, self.beamline = XChemMain.getVisitAndBeamline(
            self.processedDir)
#        print 'visit'
        self.projectDir = projectDir
        self.Logfile = XChemLog.updateLog(xce_logfile)
...
#                        ]
    ...
    def getExistingSamples(self):
        ...
        for item in allEntries:
            if str(item[0]) not in existingSamples:
                ...
                self.Logfile.insert('%s: adding %s' %
    ...
    def createAutoprocessingDir(self, xtal, run, autoproc, proc_code):
        ...
        self.Logfile.insert('%s: checking if new directory needs to be created for %s' % (xtal, os.path.join(
            self.projectDir, xtal, 'autoprocessing', self.visit + '-' + run + autoproc + '_' + proc_code)))
        ...
        if not os.path.isdir(
            ...
            self.Logfile.insert('%s: making directory %s' % (xtal, os.path.join(
                self.projectDir, xtal, 'autoprocessing', self.visit + '-' + run + autoproc + '_' + proc_code)))
            os.mkdir(os.path.join(self.projectDir, xtal, 'autoprocessing',
    ...
    def cleanUpDir(self, xtal, run, autoproc, mtzfile, logfile, proc_code):
        ...
        os.chdir(os.path.join(self.projectDir, xtal, 'autoprocessing',
                              self.visit + '-' + run + autoproc + '_' + proc_code))
        for files in glob.glob('*'):
    ...
    def copyMTZandLOGfiles(self, xtal, run, autoproc, mtzfile, logfile, proc_code):
        ...
        if os.path.isfile(mtzfile[mtzfile.rfind('/') + 1:]):
            mtzNew = os.path.join(self.projectDir, xtal, 'autoprocessing', self.visit +
                                  '-' + run + autoproc + '_' + proc_code, mtzfile[mtzfile.rfind('/')+1:])
        # MTZ file unmerged
        if os.path.isfile(mtzfile.replace('_free.mtz', '_scaled_unmerged.mtz')):
            ...
            if not os.path.isfile(xtal+'_unmerged.mtz'):
                self.Logfile.insert('%s: copying %s' % (
                    xtal, mtzfile.replace('_free.mtz', '_scaled_unmerged.mtz')))
                os.system('/bin/cp ' + mtzfile.replace('_free.mtz',
        ...
        if not os.path.isfile(logfile[logfile.rfind('/')+1:]):
            ...
            if logfile.endswith('summary.tar.gz'):
                ...
                self.cleanUpDir(xtal, run, autoproc,
                                mtzfile, logfile, proc_code)
        ...
        if os.path.isfile(logfile[logfile.rfind('/') + 1:]):
            logNew = os.path.join(self.projectDir, xtal, 'autoprocessing', self.visit +
                                  '-' + run + autoproc + '_' + proc_code, logfile[logfile.rfind('/') + 1:])
    ...
    def copyJPGs(self, xtal, run, auto):
        ...
        if self.agamemnon:
            ...
            self.Logfile.insert('looking for images in '+os.path.join(self.processedDir.replace(
                proposal, self.visit), 'jpegs', auto, self.target, xtal, run + '*.0.png'))
            for img in glob.glob(os.path.join(self.processedDir.replace(proposal, self.visit), 'jpegs', auto, self.target, xtal, run + '*.0.png')):
                if not os.path.isfile(os.path.join(self.projectDir, xtal, 'jpg', self.visit + '-' + run, img[img.rfind('/')+1:])):
                    ...
                    os.system('/bin/cp %s %s' % (img, os.path.join(self.projectDir,
                                                                   xtal, 'jpg', self.visit + '-' + auto+'_' + run)))
        else:
            for img in glob.glob(os.path.join(self.processedDir.replace('processed', 'jpegs'), run + '*.0.png')):
                ...
                if not os.path.isfile(os.path.join(self.projectDir, xtal, 'jpg', self.visit + '-' + run, img[img.rfind('/')+1:])):
                    self.Logfile.insert('%s: copying %s' % (xtal, img))
                    os.system('/bin/cp %s %s' % (img, os.path.join(self.projectDir,
                                                                   xtal, 'jpg', self.visit + '-' + run)))
            if not found:
                for img in glob.glob(os.path.join(self.processedDir.replace('processed', 'jpegs'), xtal, run + '*.0.png')):
                    if not os.path.isfile(os.path.join(self.projectDir, xtal, 'jpg', self.visit + '-' + run, img[img.rfind('/') + 1:])):
                        self.Logfile.insert('%s: copying %s' % (xtal, img))
                        os.system(
                            '/bin/cp %s %s' % (img, os.path.join(self.projectDir, xtal, 'jpg', self.visit + '-' + run)))
...
#        return db_dict
    ...
    def parse_file_system(self):
        ...
        self.Logfile.insert(
            'checking for new data processing results in '+autoDir)
        progress = 0
...
#                    proc_code = code[code.rfind('/')+1:]
                    ...
                    self.Logfile.insert(
                        xtal + ': processed directory -> ' + proc_code)
                    if current_run+proc_code in runList:
                        continue
                    self.Logfile.insert(
                        '%s -> run: %s -> current run: %s -> %s' % (xtal, run, current_run, proc_code))
                    timestamp = datetime.fromtimestamp(
                        os.path.getmtime(run)).strftime('%Y-%m-%d %H:%M:%S')
                    ...
                    for item in self.toParse:
                        ...
                        self.Logfile.insert(
                            '%s: procDir = %s' % (xtal, procDir))
                        self.Logfile.insert(
                            '%s: logfile = %s' % (xtal, logfile))
                        self.Logfile.insert(
                            '%s: mtzfile = %s' % (xtal, mtzfile))

                        for folder in glob.glob(procDir):
                            staraniso = ''
                            self.Logfile.insert(
                                '%s: searching %s' % (xtal, folder))
                            if self.junk(folder):
                                continue
                            ...
                            self.readProcessingUpdateResults(
                                xtal, folder, logfile, mtzfile, timestamp, current_run, autoproc, proc_code)
...
#            quit()
        ...
        self.Logfile.insert(
            '====== finished parsing beamline directory ======')
