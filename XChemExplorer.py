...
class XChemExplorer(QtGui.QApplication):
    ...
    def backup_soakDB(self):
        XChemMain.backup_soakDB(os.path.join(
            self.database_directory, self.data_source_file), self.xce_logfile)
    ...
    def create_new_data_source(self):
        ...
        self.data_source_file_label.setText(os.path.join(
    ...
    def select_diffraction_data_directory(self):
        ...
        self.diffraction_data_dir_label.setText(
    ...
    def search_for_datasets(self):
        ...
        self.connect(self.work_thread, QtCore.SIGNAL(
            "update_progress_bar"), self.update_progress_bar)
        self.connect(self.work_thread, QtCore.SIGNAL(
            "update_status_bar(QString)"), self.update_status_bar)
        self.connect(self.work_thread, QtCore.SIGNAL(
            "finished()"), self.thread_finished)
    ...
    def translate_datasetID_to_sampleID(self):
        ...
        button.clicked.connect(
            self.open_csv_file_translate_datasetID_to_sampleID)
        ...
        translate.addButton(QtGui.QPushButton('Cancel'),
                ...
                if len(trans_dict) >= 1:
                    ...
                    for row in range(0, allRows):
                        ...
                        if dataset_id in trans_dict:
                            ...
                            self.datasets_reprocess_table.setItem(
                                row, 1, cell_text)
                            self.update_log.insert(
    ...
    def check_for_new_autoprocessing_or_rescore(self, rescore_only):
        ...
        if rescore_only:
            ...
            msgBox.addButton(QtGui.QPushButton(
                'No'), QtGui.QMessageBox.RejectRole)
            reply = msgBox.exec_()
            if reply == 0:
                start_thread = True
            else:
...
#                start_thread = True
    ...
    def read_pinIDs_from_gda_logs(self):
        ...
        visit, beamline = XChemMain.getVisitAndBeamline(
...
#            start_thread = True
    ...
    def set_new_reference_if_applicable(self):
        print('hallo')
        reference_root = str(
            self.reference_file_selection_combobox.currentText())
        pg_ref = ''
        ucVol_ref = 0.0
        ...
        for xtal in self.initial_model_dimple_dict:
            ...
            try:
                difference = math.fabs(
                    1 - (float(ucVol_xtal) / float(ucVol_ref))) * 100
            except ValueError:
                self.update_log.insert(
            ...
            if pg_xtal == pg_ref and difference < self.allowed_unitcell_difference_percent:
                ...
                self.update_log.insert(
                    xtal + ' -> setting ' + reference_root + ' as input PDB file for DIMPLE')
    ...
    def refresh_reference_file_list(self):
        ...
        self.populate_reference_combobox(
            self.reference_file_selection_combobox)
...
#        self.ground_state_pdb_button_label.setText(pdb)
    ...
    def auto_select_ground_state_reference_PDB(self):
        ...
        for dirs in glob.glob(os.path.join(self.panddas_directory, 'processed_datasets', '*')):
            ...
            if os.path.isfile(os.path.join(dirs, xtal+'-pandda-input.pdb')):
                ...
                try:
                    xtalList.append(
                        [xtal, float(pdbHeader['Rfree']), float(pdbHeader['ResolutionHigh'])])
                except ValueError:
                    self.update_log.error(
                        '%s: cannot read Rfree or Resolution from PDB header; skipping...')
                    pass
        self.update_log.insert('found %s PDB files in %s' % (
            str(len(xtalList)), os.path.join(self.panddas_directory, 'processed_datasets')))
        if len(xtalList) >= 10:
            self.update_log.insert(
            ...
            for item in rfree:
                self.update_log.insert('%s: Rfree = %s | Resolution = %s' % (
            ...
            self.update_log.insert('selected the following PDB file: %s: Rfree = %s | Resolution = %s' % (
                reso[0][0], str(round(reso[0][1], 3)), str(round(reso[0][2], 2))))
            pdb = os.path.join(self.panddas_directory, 'processed_datasets',
                               reso[0][0], reso[0][0]+'-pandda-input.pdb')
            mtz = os.path.join(self.panddas_directory, 'processed_datasets',
                               reso[0][0], reso[0][0]+'-pandda-input.mtz')
        else:
            self.update_log.error('found less than 10 valid PDB files in %s' % os.path.join(
                self.panddas_directory, 'processed_datasets'))
        return pdb, mtz

    def prepare_ground_state_mmcif(self):
        self.update_log.insert(
            'preparing mmcif file for apo structure deposition')
    ...
    def settings_button_clicked(self):
        if self.sender().text() == 'Select Project Directory':
            self.initial_model_directory = str(
                QtGui.QFileDialog.getExistingDirectory(self.window, "Select Directory"))
            self.initial_model_directory_label.setText(
                self.initial_model_directory)
            self.pandda_input_data_dir_entry.setText(
                self.initial_model_directory)
        ...
        if self.sender().text() == 'Select Data Collection Directory':
            ...
            if dir_name != self.beamline_directory:
                ...
                self.populate_target_selection_combobox(
        ...
        if self.sender().text() == 'Select Existing\nCollection Summary File':
            ...
            self.datasets_summary_file_label.setText(
                self.datasets_summary_file)
        ...
        if self.sender().text() == 'Select CCP4_SCR Directory':
            self.ccp4_scratch_directory = str(
                QtGui.QFileDialog.getExistingDirectory(self.window, "Select Directory"))
            self.ccp4_scratch_directory_label.setText(
        ...
        if self.sender().text() == 'Select HTML Export Directory':
            self.html_export_directory = str(
                QtGui.QFileDialog.getExistingDirectory(self.window, "Select Directory"))
            self.html_export_directory_label.setText(
        ...
        if self.sender().text() == 'Select Group deposition Directory':
            self.group_deposit_directory = str(
                QtGui.QFileDialog.getExistingDirectory(self.window, "Select Directory"))
            self.group_deposition_directory_label.setText(
    ...
    def select_sample_for_dimple(self):
        ...
        for index in sorted(indexes):
            ...
            self.update_log.insert(
                '{0!s} is marked for DIMPLE'.format(index.row()))
...
#        vbox.addLayout(vbox_inital_refinement)
        ...
        vbox_restraints.addWidget(QtGui.QLabel(
            'Restraints generation program:'))
        ...
        vbox_restraints.addWidget(
            self.preferences_restraints_generation_combobox)
        ...
        adjust_max_queue_jobs.textChanged[str].connect(
            self.change_max_queue_jobs)
        ...
        settings_hbox_dimple_twin_mode.addWidget(
            self.dimple_twin_mode_label_checkbox)
        ...
        hbox.addWidget(QtGui.QLabel(
            'Additional CIF file for non-standard ligand:'))
...
#        reply = mb.exec_();
    ...
    def dimple_change_twin_mode(self):
        if self.preferences['dimple_twin_mode']:
            ...
            self.update_log.insert(
                'changing preferences: changing DIMPLE to TWIN mode')
...
#            self.xce_logfile)
    ...
    def prepare_models_for_deposition_ligand_bound(self, structureType):
        start_thread = True
        self.update_log.insert(
            'preparing mmcif files for PDB group deposition...')
        ignore_event_map = False
        if structureType == 'ground_state':
            try:
                self.update_log.insert('ground-state deposition')
                data_template_dict = self.db.get_deposit_dict_for_sample(
                    'ground_state')
...
#        structureType = "ligand_bound"
    ...
    def check_smiles_in_db_and_pdb(self):
        ...
        self.connect(self.work_thread, QtCore.SIGNAL(
            "finished()"), self.thread_finished)
        self.connect(self.work_thread, QtCore.SIGNAL(
            "show_error_dict"), self.show_error_dict)
        self.work_thread.start()
...
#        PIroles = ['group leader', 'principal investigator/group leader', 'investigator']
        ...
        grid.addWidget(QtGui.QLabel(
            'Release Status for coordinates/ SF'), 8, 0)
        ...
        grid = QtGui.QGridLayout()
        grid.addWidget(QtGui.QLabel(
            'Deposition authors (e.g. Surname, F.M.)'), 0, 0)
...
#        self.molecule_name.setStyleSheet("background-color: rgb(192, 192, 192);")
        ...
        self.fragment_name_one.setStyleSheet(
            "background-color: rgb(192, 192, 192);")
        grid.addWidget(self.fragment_name_one, 3, 1)
        grid.addWidget(QtGui.QLabel(
            '(e.g. ligand binding domain, hairpin)'), 3, 2)
        ...
        grid.addWidget(QtGui.QLabel(
            'Expression system scientific name'), 10, 0)
        ...
        grid.addWidget(QtGui.QLabel(
            'Entity 2 (IMPORTANT: only fill in if you are working with a protein-protein complex!)'), 1, 0)
...
#        self.molecule_name_two.setStyleSheet("background-color: rgb(192, 192, 192);")
        ...
        self.fragment_name_two.setStyleSheet(
            "background-color: rgb(192, 192, 192);")
        ...
        taxonomy_dict = XChemMain.NCBI_taxonomy_ID()
        for item in taxonomy_dict:
            self.Source_organism_scientific_name_two.addItem(
                taxonomy_dict[item])
        ...
        for item in taxonomy_dict:
            self.Expression_system_scientific_name_two.addItem(
                taxonomy_dict[item])
        ...
        grid.addWidget(QtGui.QLabel(
            '(e.g. beta barrel, protein-DNA complex)'), 1, 2)
...
#        grid.addWidget(QtGui.QLabel('(e.g. beta barrel, protein-DNA complex)'), 1, 2)
        ...
        grid.addWidget(QtGui.QLabel(
            '(e.g.  1 for monomer, 2 for dimer ..)'), 3, 2)
...
#        grid.addWidget(QtGui.QLabel('(Important: only for protein-protein complex'), 10, 1)
        ...
        grid.addWidget(QtGui.QLabel(
            'Sequence UNIPROT ID (Entity 2) - optional'), 13, 0)
        ...
        grid.addWidget(QtGui.QLabel(
            '(e.g. Structural Genomics Consortium)'), 23, 2)
        ...
        grid.addWidget(QtGui.QLabel(
            '(e.g. SINGLE WAVELENGTH, MAD, ...)'), 15, 2)
...
#            print "update labelTable set Label='%s',Description='%s' where ID=%s" %(label,description,str(n+1))
            self.db.execute_statement("update labelTable set Label='%s',Description='%s' where ID=%s" % (
...
#        print self.deposit_dict
        for key in self.get_deposit_dict_template():
            if key not in self.deposit_dict:
                self.update_log.warning(
                    'field not in .deposit file: ' + str(key))
    ...
    def update_deposit_input(self):
        try:
            ...
            self.contact_author_email.setText(
                self.deposit_dict['contact_author_email'])
            self.contact_author_address.setText(
                self.deposit_dict['contact_author_address'])
            self.contact_author_city.setText(
                self.deposit_dict['contact_author_city'])
            self.contact_author_State_or_Province.setText(
            ...
            self.contact_author_ORCID.setText(
                self.deposit_dict['contact_author_ORCID'])
            ...
            self.Source_organism_gene.setText(
                self.deposit_dict['Source_organism_gene'])
            self.Source_organism_strain.setText(
...
#            try:
            self.molecule_chain_one.setText(
                self.deposit_dict['molecule_chain_one'])
            # entity 2
            self.molecule_name_two.setText(
                self.deposit_dict['molecule_name_two'])
            self.fragment_name_two_specific_mutation.setText(
            ...
            self.molecule_chain_two.setText(
                self.deposit_dict['molecule_chain_two'])
            self.molecule_one_letter_sequence_uniprot_id_two.setText(
...
#                self.molecule_one_letter_sequence_two.setText('')
            ...
            self.structure_keywords.setText(
                self.deposit_dict['structure_keywords'])
            self.biological_assembly_chain_number.setText(
            ...
            self.crystallization_pH.setText(
                self.deposit_dict['crystallization_pH'])
            self.crystallization_temperature.setText(
            ...
            self.data_collection_date.setText(
                self.deposit_dict['data_collection_date'])
            self.data_collection_temperature.setText(
            ...
            self.pdbx_starting_model.setText(
                self.deposit_dict['pdbx_starting_model'])
    ...
    def update_deposit_dict(self):
        ...
        if str(self.pdbx_funding_organization_one.text()).replace(' ', '') != '':
            ...
            pdbx_grant_country_one = str(
                self.pdbx_grant_country_one.currentText())
        ...
        if str(self.pdbx_funding_organization_two.text()).replace(' ', '') != '':
            ...
            pdbx_grant_country_two = str(
                self.pdbx_grant_country_two.currentText())
        ...
        if str(self.pdbx_funding_organization_three.text()).replace(' ', '') != '':
            ...
            pdbx_grant_country_three = str(
                self.pdbx_grant_country_three.currentText())
    ...
    def set_primary_citation_as_structure_authors(self, state):
        if state == QtCore.Qt.Checked:
            for n, entry in enumerate(self.structure_author_name_List):
                self.primary_citation_author_name_List[n].setText(
                    str(entry.text()))
        else:
    ...
    def set_second_cif_file(self):
        ...
        self.update_log.insert(
            'user selected %s as CIF file for merging into ligand CIF files' % self.second_cif_file)
    ...
    def select_datasource_columns_to_display(self):
        ...
        columns_to_show.addButton(QtGui.QPushButton(
            'OK'), QtGui.QMessageBox.YesRole)
        columns_to_show.addButton(QtGui.QPushButton(
    ...
    def combo_selected(self, text):
        self.map_url = str(self.panddas_directory +
                           '/analyses/html_summaries/pandda_map_' + text + '.html')
    ...
    def add_map_html(self):
        ...
        self.connect(self.pandda_map_list, QtCore.SIGNAL(
            'activated(QString)'), self.combo_selected)
...
#        self.pandda_input_data_dir_entry.setText(os.path.join(self.initial_model_directory, '*'))
        ...
        for current_key in key_list:
            try:
                command = str(
                    'self.' + current_key + " = pickled_settings['" + key_list[current_key] + "']")
                exec(command)
                command = str(
    ...
    def update_reference_files(self, reference_root):
        ...
        self.populate_reference_combobox(
            self.reference_file_selection_combobox)
        self.populate_reference_combobox(
            self.pandda_reference_file_selection_combobox)
    ...
    def rerun_dimple_on_all_autoprocessing_files(self):
        job_list = []
        self.update_log.insert(
            'preparing to run DIMPLE on all autoprocessing files')
        ...
        if job_list:
            self.update_log.insert(
                'trying to run DIMPLE on ALL auto-processing files')
                ...
                if os.path.isfile(
                    ...
                    if os.path.isfile(
                        ...
                        mtzin = os.path.join(db_dict['ProjectDirectory'], xtal, db_dict['DataProcessingPathToMTZfile'],
                            ...
                            os.path.join(db_dict['ProjectDirectory'], xtal, db_dict['DataProcessingPathToMTZfile'])):
                        mtzin = os.path.join(
                            db_dict['ProjectDirectory'], xtal, db_dict['DataProcessingPathToMTZfile'])
    ...
    def remove_selected_dimple_files(self, instruction):
        ...
        if job_list:
            msgBox = QtGui.QMessageBox()
            msgBox.setText("Do you really want to delete {0!s} {1!s} files?".format(
                len(job_list), self.preferences['initial_refinement_pipeline']))
            msgBox.addButton(QtGui.QPushButton(
                'Go'), QtGui.QMessageBox.YesRole)
            msgBox.addButton(QtGui.QPushButton('Cancel'),
                             QtGui.QMessageBox.RejectRole)
            reply = msgBox.exec_()
    ...
    def set_results_from_selected_pipeline(self, instruction):
        ...
        self.update_log.warning(
            'selecting initial refinement results from '+pipeline)
    ...
    def run_xia2_on_selected_datasets(self, overwrite):
        ...
        if str(self.reprocess_isigma_combobox.currentText()) != 'default':
            reso_limit.append(
                str(self.reprocess_isigma_combobox.currentText()))
    ...
    def get_job_list_for_dimple_rerun(self, xtal, job_list, db_dict, entry):
        ...
        for reference in self.reference_file_list:
            ...
            if reference[5] == db_dict['DataProcessingPointGroup']:
                try:
                    difference = math.fabs(
                        1 - (float(db_dict['DataProcessingUnitCellVolume']) / float(reference[4])))
        ...
        if suitable_reference:
            ...
            if os.path.isfile(os.path.join(self.initial_model_directory, xtal, xtal + '.mtz')):
                mtzin = os.path.join(
                    self.initial_model_directory, xtal, xtal + '.mtz')
    ...
    def update_datasets_reprocess_table(self, data_dict):
        ...
        self.diffraction_data_search_info = 'found ' + \
            str(len(self.diffraction_data_dict)) + ' datasets'
        self.diffraction_data_search_label.setText(
        ...
        for entry in sorted(self.diffraction_data_dict):
            ...
            for column, header in enumerate(column_name):
                if header[0] == 'Dataset ID' or header[0] == 'Sample ID':
                    ...
                    run_xia2.toggle()
                    self.datasets_reprocess_table.setCellWidget(
                        row, column, run_xia2)
                    ...
                    if db_dict != {}:
                        if header[0] == 'DataProcessing\nStatus':
                            if str(db_dict[header[1]]) == 'running':
                                cell_text.setBackground(
                                    QtGui.QColor(100, 230, 150))
                            elif str(db_dict[header[1]]) == 'pending':
                                cell_text.setBackground(
                                    QtGui.QColor(20, 100, 230))
                            elif str(db_dict[header[1]]) == 'started':
                                cell_text.setBackground(
                                    QtGui.QColor(230, 240, 110))
    ...
    def button_clicked(self):
        if not self.data_source_set:
            ...
            if self.sender().text() == "Create New Data\nSource (SQLite)":
                ...
                if self.data_source_file == '':
                    self.database_directory = file_name[:file_name.rfind('/')]
                    self.data_source_file = file_name[file_name.rfind(
                        '/') + 1:]
                    self.data_source_file_label.setText(os.path.join(
                        self.database_directory, self.data_source_file))
                    self.settings['database_directory'] = self.database_directory
        ...
        for item in self.workflow_widget_dict:
            for widget in self.workflow_widget_dict[item]:
                if widget == self.sender():
                    ...
                    instruction = str(
                        self.workflow_widget_dict[item][0].currentText())
                    print(instruction)
                    ...
                    if self.main_tab_widget.currentIndex() == task_index:
                        if self.explorer_active == 0 and self.data_source_set == True:
                            if action == 'Run':
                                ...
                                self.get_status_of_workflow_milestone(
                                    instruction)
    ...
    def get_status_of_workflow_milestone(self, instruction):
        ...
        self.update_log.insert(
            '{0!s} samples are currently in database'.format(str(len(self.xtal_db_dict))))
...
#            self.run_refine_bound_state_with_buster_new('new')
            ...
            if not self.coot_running:
                ...
                if instruction == "Open COOT":
                    ...
                    interface = 'old'
#                    print self.settings
                self.work_thread = XChemThread.start_COOT(
    ...
    def run_pandda_analyse(self, run):
        ...
        if run == 'pre_run':
            ...
            vbox.addWidget(QtGui.QLabel(
                XChemToolTips.pandda_pre_run(self.reference_directory)))
            hbox = QtGui.QHBoxLayout()
    ...
    def convert_event_maps_to_SF(self):
        self.update_log.insert('converting all event maps in {0!s} to mtz files'.format(
            self.initial_model_directory))
    ...
    def run_pandda_export(self, update_datasource_only, which_models):
        ...
        if which_models == 'all':
            ...
            if reply == 0:
                if update_datasource_only:
                    self.update_log.insert(
                        'will update panddaTable in database only')
                else:
...
#            self.work_thread.start()
    ...
    def show_pandda_html_summary(self):
        self.pandda_initial_html.load(
            QtCore.QUrl(self.pandda_initial_html_file))
        self.pandda_initial_html.show()
        self.pandda_analyse_html.load(
            QtCore.QUrl(self.pandda_analyse_html_file))
        self.pandda_analyse_html.show()
        self.add_map_html()
        self.pandda_inspect_html.load(
            QtCore.QUrl(self.pandda_inspect_html_file))
            ...
            if todo == 'ALL':
                ...
                if not os.path.isfile(os.path.join(self.initial_model_directory, str(item[0]), compoundID + '.cif')):
                    compound_list.append(
                        [str(item[0]), compoundID, str(item[2])])
            elif todo == 'SELECTED':
    ...
    def merge_cif_files(self, todo):
        ...
        if todo == 'merge':
            self.update_log.insert(
                'trying to merge %s with ligand restraint files in project directory' % self.second_cif_file)
        elif todo == 'restore':
        ...
        if todo == 'merge':
            if os.path.isfile(str(self.second_cif_file)):
                self.update_log.insert(
                    'checking compound code of second CIF file (%s)' % self.second_cif_file)
                self.update_log.insert('Note: LIG and DRG are not allowed!')
                import iotbx.cif
                cif_model = iotbx.cif.reader(
                    file_path=self.second_cif_file).model()
                cif_block = cif_model["comp_list"]
                ligID = cif_block["_chem_comp.id"]
                self.update_log.insert(
                    'found the following compound codes in the supplied CIF file: %s' % str(list(ligID)))
                ...
                self.update_log.error(
                    XChemToolTips.second_cif_file_not_exists())
                start_thread = False

            if start_thread:
                msgBox = QtGui.QMessageBox()
                msgBox.setText(XChemToolTips.second_cif_file_info(
    ...
    def update_deposition_table(self):
        ...
        if mismatch != {}:
            ...
            for entry in mismatch:
                self.update_log.insert(
                    entry[0] + ' -> site: ' + entry[1] + ' @ ' + entry[2] + ' => ' + entry[4])
            self.update_log.insert(
                'You need to change this before you can continue!')
            return None
    ...
    def need_to_switch_main_tab(self, task_index):
        msgBox = QtGui.QMessageBox()
        msgBox.setText(
            "Need to switch main tab before you can launch this job")
        msgBox.addButton(QtGui.QPushButton('Yes'), QtGui.QMessageBox.YesRole)
        msgBox.addButton(QtGui.QPushButton('No'), QtGui.QMessageBox.RejectRole)
        reply = msgBox.exec_()
        if reply == 0:
    ...
    def create_widgets_for_autoprocessing_results_only(self, data_dict):
        ...
        for xtal in sorted(self.data_collection_dict):
            ...
            for entry in self.data_collection_dict[xtal]:
                if entry[0] == 'image':
                    tmp.append([entry[3], datetime.strptime(
                        entry[3], '%Y-%m-%d %H:%M:%S')])
    ...
    def create_maps_table(self):
        ...
        for xtal in sorted(self.xtal_db_dict):
            ...
            if str(db_dict['DataCollectionOutcome']).lower().startswith('success'):
                ...
                smallest_uc_difference = min(
                    reference_file, key=lambda x: x[1])
                    ...
                    if header[0] == 'Sample ID':
                        ...
                        if header[0] == 'Compound\nStatus':
                            if str(db_dict[header[1]]) == 'running':
                                ...
                                cell_text.setBackground(
                                    QtGui.QColor(240, 150, 20))
                        self.maps_table.setItem(current_row, column, cell_text)
            if new_xtal:
                self.initial_model_dimple_dict[xtal] = [
                    run_dimple, reference_file_selection_combobox]
    ...
    def preferences_selection_mechanism_combobox_changed(self, i):
        ...
        self.update_log.insert(
            'setting datasets selection mechanism to ' + text)
    ...
    def preferences_initial_refinement_combobox_changed(self, i):
        ...
        self.update_log.insert(
            'setting initial refinement pipeline to ' + text)
    ...
    def preferences_restraints_generation_combobox_changed(self):
        ...
        self.update_log.insert(
            'will use {0!s} for generation of ligand coordinates and restraints'.format(text))
    ...
    def get_reference_file_list(self, reference_root):
        ...
        if os.path.isfile(os.path.join(self.reference_directory, reference_root + '.pdb')):
            pdb_reference = parse().PDBheader(os.path.join(
            ...
            for files in glob.glob(self.reference_directory + '/*'):
                if files.endswith('.pdb'):
                    reference_root = files[files.rfind(
                        '/') + 1:files.rfind('.')]
        ...
        for n, file in enumerate(reference_file_list):
            self.update_log.insert(
                'reference file {0!s}: {1!s}'.format(n, file))
    ...
    def dataset_outcome_combobox_change_outcome(self, text):
        ...
        for key in self.dataset_outcome_combobox_dict:
            if self.dataset_outcome_combobox_dict[key] == self.sender():
                xtal = key
                self.update_log.insert(
                    'user changed data collection outcome of {0!s} to {1!s}'.format(xtal, outcome))
                break
    ...
    def show_data_collection_details(self, state):
        ...
        for table_row in range(allRows):
            tmp.append([self.datasets_summary_table.item(
                table_row, 0).text(), table_row])
        ...
        for key in self.datasets_summary_dict:
            if self.datasets_summary_dict[key][3] == self.sender():
                if self.sender().isChecked():
                    ...
                    self.data_collection_details_currently_on_display = self.data_collection_column_three_dict[
                        key][0]
...
#
        ...
        for column, header in enumerate(column_name):
            ...
            if header[0] == 'Sample ID':
                ...
                image.setPixmap(pixmap.scaled(
                    image.size(), QtCore.Qt.KeepAspectRatio))
    ...
    def populate_datasets_summary_table_NEW(self):
        ...
        collectedXtalsDict = self.db.xtals_collected_during_visit_as_dict(
            visit)

        # instead of using dictionaries, query table of which crystals are in table
        samples_in_table = self.get_sample_list_from_table(
        ...
        for xtal in sorted(collectedXtalsDict):
            if xtal not in samples_in_table:
                ...
                row = self.get_row_of_sample_in_table(
                    self.datasets_summary_table, xtal)
    ...
    def show_results_from_all_pipelines(self):
        ...
        for db_dict in dbList:
            ...
            self.update_row_in_table(
                xtal, row, db_dict, self.data_collection_table, self.data_collection_table_columns)
...
#        msgBoxLayout.addLayout(vbox, 0, 0)
        ...
        self.msgBox.addButton(QtGui.QPushButton(
            'Cancel'), QtGui.QMessageBox.RejectRole)
...
#            dbDict = self.db.get_db_dict_for_visit_run_autoproc(xtal,visit,run,autoproc)
            ...
            self.update_log.insert('%s: changing selected autoprocessing result to %s %s %s' % (
    ...
    def user_update_selected_autoproc_datasets_summary_table(self):
        for key in self.data_collection_column_three_dict:
            if self.data_collection_column_three_dict[key][0] == self.sender():
                ...
                if not user_already_changed_selection:
                    self.data_collection_dict[key].append(
    ...
    def populate_and_update_datasource_table(self):
        ...
        for row in range(current_row):
            sampleID = str(self.overview_datasource_table.item(
                row, 0).text())  # this must be the case
    ...
    def select_sample_for_pandda(self, option):
        ...
        if option == 'deselect':
            for index in sorted(indexes):
                self.pandda_analyse_data_table.cellWidget(
                    index.row(), 6).setChecked(False)
                self.pandda_analyse_data_table.cellWidget(
                    index.row(), 7).setChecked(False)
                self.pandda_analyse_data_table.cellWidget(
                    index.row(), 8).setChecked(False)
        else:
            for index in sorted(indexes):
                self.pandda_analyse_data_table.cellWidget(
                    index.row(), 6).setChecked(False)
                ...
                self.pandda_analyse_data_table.cellWidget(
                    index.row(), 8).setChecked(False)
                if option == 'ignore':
                    checkbox = self.pandda_analyse_data_table.cellWidget(
                        index.row(), 6)
                if option == 'char':
                    checkbox = self.pandda_analyse_data_table.cellWidget(
                        index.row(), 7)
...
#                            self.refinement_table.setItem(current_row, column, cell_text)
    ...
    def update_data_source(self, sample, db_dict):
        data_source = XChemDB.data_source(os.path.join(
            self.database_directory, self.data_source_file))
    ...
    def quit_xce(self):
        ...
        if self.data_collection_dict != {}:
            if os.path.isfile(self.datasets_summary_file):
                ...
                pickle.dump(self.data_collection_dict, open(
                    self.datasets_summary_file, 'wb'))
