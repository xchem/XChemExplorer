...
class Refine(object):
    ...
    def make_covalent_link(self, covLinkAtomSpec, Logfile):
        ...
        # Build the ligand + link restraints with acedrg from the latest CCP4
        # (ccp4/9) instead of the pinned ccp4/7.1.018: the 7.1.018 acedrg is
        # several years out of date and its covalent-link handling has been
        # improved upstream since. This runs in a throwaway subshell (os.system
        # -> /bin/sh, which inherits the exported `module` function) so the rest
        # of XCE stays in the 7.1.018 environment it depends on.
        #   acedrg -c regenerates the ligand dict (acedrg dislikes some
        #   descriptions produced by phenix.elbow);
        #   acedrg -L then generates the link restraints.
        os.system(
            "module unload ccp4 2>/dev/null; module load ccp4/9 2>/dev/null; "
            "acedrg -c %s.cif -o %s_acedrg; "
            "acedrg -L covalent_bond.txt -o %s"
            % (self.compoundID, self.compoundID, self.compoundID)
        )
