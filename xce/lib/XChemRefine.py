...
class Refine(object):
    ...
    def make_covalent_link(self, covLinkAtomSpec, Logfile):
        ...
        if residue_2 in residuesToModify:
            bond_text = (
                "LINK: RES-NAME-1 %s FILE-1 %s_acedrg.cif ATOM-NAME-1  %s  RES-NAME-2 %s ATOM-NAME-2  %s"
    ...
    def clean_up_before_refinement(self):
        ...
        os.chdir(os.path.join(self.ProjectPath, self.xtalID))
        os.system(
            "/bin/rm refine.pdb refine.mtz refine.split.bound-state.pdb validation_summary.txt validate_ligands.txt 2fofc.map fofc.map refine_molprobity.log"
        )
    ...
    def run_giant_score_model(self, cmd, cycle):
        old_ccp4 = ""
        if os.path.isfile(
            "/dls/science/groups/i04-1/software/pandda_0.2.12/ccp4/ccp4-7.0/bin/ccp4.setup-sh"
        ):
    ...
    def RunQuickRefine(
        ...
        if os.path.isfile(
            ...
            if os.path.isfile(
                ...
                Logfile.warning(
                    "%s: ground-state model does not exist, but refine.modified.pdb does exist"
                    % self.xtalID
                )
                Logfile.warning(
                ...
                Logfile.warning(
                    "creating symbolic link: ln -s refine.modified.pdb %s-ensemble-model.pdb"
                    % self.xtalID
                )
                os.system(
        ...
        if os.path.isfile(
            ...
            try:
                ...
                Logfile.insert(
                    "checking if %s-ensemble-model.pdb contains residue of type LIG, DRG, FRG, UNK or UNL"
                    % self.xtalID
                )
                ...
                if found_lig:
                    Logfile.warning(
                        "giant.make_restraints was not able to create multi-state-restraints.refmac.params. "
                    ...
                    Logfile.error(
                        "%s-ensemble-model.pdb does not contain any modelled ligand; aborting refinement"
                        % self.xtalID
                    )
                    return None
            except OSError:
