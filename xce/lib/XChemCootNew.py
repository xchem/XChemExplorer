...
class GUI(object):
    ...
    def RefreshData(self):
        ...
        self.panddaSerial = panddaSerial = m = (4 - len(str(self.Serial))) * "0" + str(
            self.Serial
        )
        ...
        if self.refinementProtocol.startswith("pandda"):
            self.Logfile.insert(
                "==> COOT: looking for ground-state model "
                + os.path.join(
                    self.project_directory,
                    self.xtalID,
            ...
            self.Logfile.insert(
                "==> COOT: looking for bound-state model "
                + os.path.join(
                    self.project_directory,
                    self.xtalID,
        ...
        if os.path.isfile(
            os.path.join(self.project_directory, self.xtalID, "2fofc_??????.map")
        ):
            coot.set_colour_map_rotation_on_read_pdb(0)
    ...
    def show_potential_link(self, *clicks):
        ...
        for imol in coot_utils_XChem.molecule_number_list():
            ...
            if (
                ...
                or coot.molecule_name(imol).endswith("init.pdb")
