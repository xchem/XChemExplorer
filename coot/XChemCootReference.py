...
class GUI(object):
    ...
    def RefreshData(self):
        ...
        try:
            self.RRfreeBox.modify_bg(
                gtk.STATE_NORMAL, gtk.gdk.color_parse(self.QualityIndicators["RfreeTL"])
            )
        except ValueError:
            pass
        ...
        try:
            self.MolprobityScoreBox.modify_bg(
                gtk.STATE_NORMAL,
                gtk.gdk.color_parse(self.QualityIndicators["MolprobityScoreColor"]),
            )
        except ValueError:
            pass
        ...
        try:
            self.RamachandranFavoredBox.modify_bg(
                gtk.STATE_NORMAL,
                gtk.gdk.color_parse(self.QualityIndicators["RamachandranFavoredColor"]),
            )
        except ValueError:
            pass
    ...
    def REFINE(self, widget):
        ...
        while os.path.exists(
            os.path.join(
                self.reference_directory, self.refinementDir, "REFINEMENT_IN_PROGRESS"
            )
        ):
            time.sleep(10)
            print(
                "==> XCE: waiting for refinement to finish; elapsed time = "
                + str(snooze)
                + "s"
            )
            snooze += 10
    ...
    def update_pdb_mtz_files(self, pdbRoot):
        ...
        if len(coot_utils_XChem.molecule_number_list()) > 0:
            for item in coot_utils_XChem.molecule_number_list():
                if coot.molecule_name(item).endswith(
                    ".pdb"
                ) or ".mtz" in coot.molecule_name(item):
        ...
        print(
            "REFI_DIR",
            os.path.join(self.reference_directory, self.refinementDir, "refine.pdb"),
        )
        if os.path.isfile(
            os.path.join(self.reference_directory, self.refinementDir, "refine.pdb")
        ):
            ...
            os.symlink(
                os.path.realpath(os.path.join(self.refinementDir, "refine.pdb")),
                "%s" % Root,
            )
        ...
        groundStateMap = os.path.join(
            self.reference_directory, Root + "-mean-map.native.ccp4"
        ).replace(".pdb", "")
